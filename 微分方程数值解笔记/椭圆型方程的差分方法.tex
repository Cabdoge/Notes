...
其中$\boldsymbol{M}$是非奇异矩阵.

那么上述线性代数方程组可以转化为
\[\boldsymbol{M}\boldsymbol{x} = \boldsymbol{N}\boldsymbol{x} + \boldsymbol{b}\]\par
从而得到如下迭代法：
...
\begin{minipage}{0.49\textwidth}
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/1.3.png}
...
\begin{minipage}{0.49\textwidth}
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{fig/1.4.png}
...
\noindent\textbf{方法一：}将未知矩阵拉直获得列向量，形成线性方程组
...
我们知道，若$A=f(B)$，其中$f$是多项式，且$\lambda$为$B$的特征值，那么$f(\lambda)$是$A$的特征值.\par
