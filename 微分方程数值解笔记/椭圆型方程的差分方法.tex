...
给定线性代数方程组：$\boldsymbol{A}\boldsymbol{x} = \boldsymbol{b}$，式中 $\boldsymbol{A}$ 为非奇异矩阵.\par
将 $\boldsymbol{A}$ 作矩阵分裂可得${\color{red} \boldsymbol{A} = \boldsymbol{M} - \boldsymbol{N}}$，式中 $\boldsymbol{M}$ 是非奇异矩阵.\par
...
式中,
\[ \Lambda_1 = \mathrm{diag}(\lambda_1, \cdots, \lambda_I), \quad \Lambda_2 = \mathrm{diag}(\mu_1, \cdots, \mu_J). \]
于是矩阵方程可重写为
...
\begin{enumerate}[label=步 \arabic*.]
    ...
    \item 计算矩阵 $W = [w_{ij}]$, 
    \[ w_{ij} = \frac{4v_{ij}}{(\lambda_i + \mu_j)(I+1)(J+1)}. \]
...
\begin{equation}
    \delta_x^2 u_i = u''(x_i) + \frac{1}{12}h^2 u^{(4)}(x_i) + \frac{2}{6!}h^4 u^{(6)}(\xi_i), \tag{3.1}
\end{equation}
以及
\begin{equation}
    \delta_x^2 v_i = v''(x_i) + \frac{1}{12}h^2 v^{(4)}(\eta_i). \tag{3.2}
\end{equation}\par
...
即
\begin{equation}
    \mathcal{W}_x v_i = \frac{1}{12}(v_{i-1} + 10v_i + v_{i+1}), \tag{3.3}
\end{equation}
则
\begin{equation}
    \mathcal{W}_x u''(x_i) = \delta_x^2 u_i + \rho_i(h), \tag{3.4}
\end{equation}
...
注意到 $\bm{G}$ 的表达式, 可推得
\begin{equation}
    \langle \bm{G}, \bm{p}_j \rangle = G_1 \sin(\theta_j) + G_N \sin(N\theta_j), \tag{3.6}
\end{equation}
