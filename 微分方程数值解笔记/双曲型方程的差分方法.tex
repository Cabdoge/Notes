...
考虑
\begin{equation*}
    u_j^{n+1}-\frac{1}{2}\ab(u_{j+1}^n+u_{j-1}^n)+\frac{1}{2}a\lambda\ab(u_{j+1}^n-u_{j-1}^n)=0
\end{equation*}
进一步化简并令$u^{n}=v^n\eu^{\iu jkh}$，则
\begin{equation*}
    v^{n+1}=\frac{1}{2}\ab((1-a\lambda)\eu^{\iu kh}+(1+a\lambda)\eu^{-\iu kj})v^n
\end{equation*}
...
\begin{equation*}
    u^{n+1/2}_{j}\approx \frac{1}{2}\ab(u^{n+1}_{j} + u^{n}_{j})\approx \frac{1}{4h}\ab(u^{n+1}_{j+1} - u^{n+1}_{j-1} + u^{n}_{j+1} - u^{n}_{j-1}).
\end{equation*}
...
\begin{instance}
    ...
    \begin{equation*}
        \frac{u^{n+1}_j-u^n_j}{\tau}=\frac{a}{2}\ab(\delta^2_x u^{n+1}_j+\delta^2_x u^n_j),
    \end{equation*}
...
将方程在网格点 $(x_j, t_n)$领域中，将系数 $a(x,t)$ 冻结为常数 $a(x_j, t_n)=a_{j}^n$ 而得
\begin{equation*}
    u_t + a(x_j, t_n)u_x = 0,
\end{equation*}
...
化简后即得递推关系为
\begin{equation*}
    u_j^{n+1} = (1 - a_{j}^n\lambda) u_j^n + a_{j}^n\lambda u_{j-1}^n.
\end{equation*}
...
\textbf{方法 2. 特征线方法.}\par
\noindent\allbf{Step 1.} 过$P(x_j,t_{n+1})$作特征线$L:x=x(t)$与$t=t_n$交于点$Q(x_Q,t_n)$
对流问题\eqref{eq:variable-coefficient-hyperbolic}过 $(\bar{x}, \bar{t})$ 的特征线方程满足
$\frac{\d u}{\d t}=0$，故
...
\noindent\allbf{Step 2.} 计算$u(x_Q,t_n)$的近似值.需要确定$x_Q$.\par
记$x=x(t;\bar{x},\bar{t})$，则
\begin{equation*}
    x_Q=x(t_n; x_j, t_{n+1}).
\end{equation*}

对特征线方程 \eqref{eq:characteristic-variable} 在 $[t_n, t_{n+1}]$ 积分, 并注意到 $t = t_{n+1}$ 时, $x = x_j$, 可有
\begin{equation*}
    x_j - x_Q = \int_{t_n}^{t_{n+1}} a(x,t) \,\mathrm{d}t.
\end{equation*}

根据数值积分方法, 上式的右端可以有多种近似方式, 从而得到不同的差分格式. 例如, 可以使用如下数值积分方法来近似上述积分:
\begin{enumerate}[(1)]
    \item 右矩形公式.
    \begin{equation*}
        x_j - x_Q \approx \tau a(x_j, t_{n+1}) \implies x_Q \approx x_j - \tau a(x_j, t_{n+1}).
    \end{equation*}
    \item 左矩形公式.
    \begin{equation*}
        x_j - x_Q \approx \tau a(x_Q, t_n) \implies \text{求得 } x_Q \text{ 的近似值.}
    \end{equation*}
    \item 梯形公式.
    \begin{equation*}
        x_j - x_Q \approx \frac{\tau}{2}(a(x_j, t_{n+1}) + a(x_Q, t_n)) \implies \text{求得 } x_Q \text{ 的近似值.}
    \end{equation*}
\end{enumerate}
而后利用
\begin{equation}
    u_j^{n+1} \approx u(x_j, t_{n+1}) = u(x(t_n; x_j, t_{n+1}), t_n)=u(x_Q, t_n)
\end{equation}
即可得到$u(x_j,t_{n+1})$的值.

\textbf{核心思想：}利用特征线方程, 将偏微分方程数值求解转化为常微分方程数值求解.

\subsection{差分格式的稳定性分析}

对于上述差分方法，必须也要分析其稳定性.但由于此时方程是变系数的，因此无法使用Fourier方法来判断稳定性.此时我们引入能量方法研究差分格式的稳定性. 设 $a(x,t) > 0$, 考虑求解问题 \eqref{eq:variable-coefficient-hyperbolic} 的差分格式 \eqref{eq:upwind-variable} 的稳定性. 该格式可重写为
\begin{equation}\label{eq:变系数迎风化简}
...
我们的目的是证明：
\begin{equation}
    \|\bm{u}^n\|_h^2 \leq k\|\bm{u}^0\|_h^2,
\end{equation}
在差分方程\eqref{eq:变系数迎风化简}两侧同乘以 $u_j^{n+1}$, 再利用 Cauchy-Schwarz 不等式（在这里等价于基本不等式 $2xy \le x^2 + y^2$）可知
\begin{equation*}
    \begin{aligned}
        (u_j^{n+1})^2 &= (1 - \lambda a_j^n)u_j^n u_j^{n+1} + \lambda a_j^n u_{j-1}^n u_j^{n+1} \\
        &\leq \frac{1 - \lambda a_j^n}{2} \{ (u_j^n)^2 + (u_j^{n+1})^2 \} + \frac{\lambda a_j^n}{2} \{ (u_{j-1}^n)^2 + (u_j^{n+1})^2 \},
    \end{aligned}
\end{equation*}
上述不等式成立需要满足系数非负，即要求 $1 - \lambda a_j^n \geq 0$ 且 $\lambda a_j^n \geq 0$，即$\lambda \max_{j} a_j^n \leq 1$. 整理后得
\begin{equation*}
    (u_j^{n+1})^2 \leq (1 - \lambda a_j^n)(u_j^n)^2 + \lambda a_j^n(u_{j-1}^n)^2.
\end{equation*}
又
\begin{align*}
    \|\bm{u}^{n+1}\|_h^2 &= h\sum_{j=-\infty}^\infty (u_j^{n+1})^2 \\
    &\leq h\sum_{j=-\infty}^\infty  (1 - \lambda a_j^n)(u_j^n)^2 + h\sum_{j=-\infty}^\infty \lambda a_{j+1}^n(u_{j}^n)^2  \\
    &=h\sum_{j=-\infty}^\infty (u_j^n)^2 + \tau\sum_{j=-\infty}^\infty (a_{j+1}^n - a_j^n)(u_j^n)^2 
\end{align*}
由Lagrange中值定理，
\begin{equation*}
    a_{j+1}^n - a_j^n = a(x_{j+1}, t_n) - a(x_j, t_n) = a_x(\xi_j, t_n)h,
\end{equation*}
从而
\begin{align*}
    \|\bm{u}^{n+1}\|_h^2 &\leq h\sum_{j=-\infty}^\infty (u_j^n)^2 + \tau h\sum_{j=-\infty}^\infty a_x(\xi_j, t_n)(u_j^n)^2 \\
    &\leq (1 + M\tau) \|\bm{u}^n\|_h^2.
\end{align*}
其中，$M=\sup_{x,t} |a_x(x,t)|$.
进一步作迭代可得对任意 $T > 0$, 当 $n\tau \leq T$ 时, 成立
\begin{equation*}
    \|\bm{u}^n\|_h^2 \leq (1 + M\tau)^n \|\bm{u}^0\|_h^2 \leq e^{MT} \|\bm{u}^0\|_h^2.
\end{equation*}
因此, 若条件$\lambda \max a_j^n\leq 1$成立时, 格式 \eqref{eq:upwind-variable} 是稳定的.

另外，可以使用最大模方法来分析该格式的稳定性.由三角不等式可得
\begin{align*}
    |u_j^{n+1}| &\leq |1-\lambda a_j^n||u_j^n| + \lambda a_j^n |u_{j-1}^n| \\
    &\leq (\lambda a_j^n+(1-\lambda a_j^n))\max_{j}\norm{\bm{u}^n}_\infty \\
    &= \norm{\bm{u}^n}_\infty.
\end{align*}
迭代后可以得到
\begin{equation*}
    \norm{\bm{u}^n}_\infty \leq \norm{\bm{u}^0}_\infty.
\end{equation*}
...
\end{equation*}
该格式的精度为 $O(\tau + h)$.\par
当$n=1$时，除了$x=0$的点外都可以由向前差商和初值条件得到.但在$x=0$处，它本应使用$x=0,n=0$的点的取值，但$x=0$没有方程假设满足，因此必须还要做出假设.一般即设想 $x = 0$ 处也满足双曲型方程, 则使用迎风格式离散可得
\begin{equation*}
    \frac{u_0^{n+1} - u_0^n}{\tau} - \frac{u_1^n - u_0^n}{h} = 0.
\end{equation*}
不断迭代即可得到方程的数值解.\par

\noindent\textbf{方法 2: 蛙跳格式.}
\begin{equation}\label{eq:leapfrog}
    \frac{u_j^{n+1} - u_j^{n-1}}{2\tau} - \frac{u_{j+1}^n - u_{j-1}^n}{2h} = 0, \quad 0 < j < J.
\end{equation}
该格式精度为 $O(\tau^2 + h^2)$，较迎风格式更高.\par
\begin{figure}[htbp]
    \centering
    ...
    \end{tikzpicture}
    \captionof{figure}{蛙跳格式示意图.}
\end{figure}
同样地, 它在 $x = 0$ 处的信息无法知道. 有两个解决方法:
\begin{enumerate}[(1)]
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{fig/3.3.png}
    \captionof{figure}{$\alpha=1$时左边界的计算结果}
\end{figure}
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{fig/3.4.png}
    \captionof{figure}{$\alpha=10$时左边界的计算结果}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{fig/3.5.png}
    \captionof{figure}{$\alpha=1$时两种方法的绝对误差(左：均匀化法，右：迎风格式法)}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{fig/3.6.png}
    \captionof{figure}{$\alpha=10$时两种方法的绝对误差(左：均匀化法，右：迎风格式法)}
\end{figure}
...
移项化简可得
\begin{equation*}
    u_j^{n+1}=2u_j^n - u_j^{n-1} + (a\lambda)^2 \{u_{j+1}^n - 2u_j^n + u_{j-1}^n\}.
\end{equation*}
它是一个三层显示格式.易得该差分方程的截断误差为 $O(\tau^2 + h^2)$.并且该格式弱不稳定的.\par
\begin{question}
    上述格式是一个三层格式，那么我们需要两个时间层的初值条件才能开始迭代，应当如何取$u_j^1$?
\end{question}
\noindent\textbf{法一：标准方法.} 考虑在$u_t$处的数值微分
\begin{equation*}
    u_1(x)=u_t(x,0)\approx \frac{u_j^1-u_j^0}{\tau}
\end{equation*}
进而
\begin{equation*}
    u_j^1 = u_0(x_j) + \tau u_1(x_j).
\end{equation*}

这一方法的问题在于通过数值微分得到的$u_j^1$只有一阶精度，和中心差商的二阶精度不相匹配.\par
\noindent\textbf{法二：虚拟格点法.} 另一种方法是引进虚拟网格 $(x_j, -\tau)$ 及格点函数 $u_j^{-1}$ (见图 \ref{fig:virtual-grid}).\par
\begin{figure}[htbp]
    \centering
...
\begin{equation*}
    \begin{cases}
        \frac{u_j^1 - u_j^{-1}}{2\tau} = u_t(x_j, 0) = u_1(x_j), \\[2ex]
...
\noindent\textbf{法三：Taylor 展开法.} 先将其连续化，有$u_j^1=u(x_j,\tau)$，由Taylor 展开得到:
\begin{equation*}
    \begin{aligned}
...
\noindent\textbf{方法 2. 高阶方程化为一阶方程组法.}
