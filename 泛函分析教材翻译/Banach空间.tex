...
(p1) 距离 \(d\) 在平移下保持不变，即:对任意 \(x,y,z \in  X\) ，均有 \(d\left( {x,y}\right)  = \; d\left( {x + z,y + z}\right)\) .特别地， \(d\left( {x,y}\right)  = \; d\left( {x - y,0}\right)\) .

(p2) 距离 \(d\) 是正齐次的，即:对任意标量 \(\lambda\) 及任意 \(x,y \in  X\) ，均有 \(d\left( {{\lambda x},{\lambda y}}\right)  = \; \left| \lambda \right| d\left( {x,y}\right)\) .

(p3) 每个开球 \(B\left( {{x}_{0},r}\right)  \doteq  \left\{  {x \in  X;d\left( {x,{x}_{0}}\right)  < r}\right\}\) 均为凸集.
...
(N2) 对每个 \(x \in  X\) 与 \(\lambda  \in  \mathbb{K}\) ，有 \(\| {\lambda x}\|  = \left| \lambda \right| \| x\|\) .

(N3) 对每个 \(x,y \in  X\) ，有 \(\| x + y\|  \leq  \| x\|  + \| y\|\) .

配备满足(N1)-(N3)的范数 \(\|  \cdot  \|\) 的向量空间 \(X\) 称为赋范空间.进而，该范数可确定 \(X\) 中元素之间的距离.

引理2.1(由范数定义的距离).设 \(\|  \cdot  \|\) 为向量空间 \(X\) 上的一个范数.则
...
距离 \(d\left( {x,y}\right)  = \| x - y\|\) 在向量空间 \(X\) 上确定了一个拓扑.因此我们可讨论开集、闭集、收敛序列及连续映射.
...
我们回顾:集合 \(V \subseteq  X\) 是开集，当且仅当对每个 \(x \in  V\) ，存在 \(r > 0\) 使得 \(B\left( {x,r}\right)  \subseteq  V\) ；集合 \(U \subseteq  X\) 是闭集，当且仅当其补集 \(X \smallsetminus  U\) 是开集.
...
\[
\begin{Vmatrix}{f\left( {x}^{\prime }\right)  - f\left( x\right) }\end{Vmatrix} < \varepsilon \;\text{ whenever }{x}^{\prime } \in  X,\begin{Vmatrix}{{x}^{\prime } - x}\end{Vmatrix} < \delta .
\]

序列 \({\left( {x}_{n}\right) }_{n \geq  1}\) 是Cauchy列，若对每个 \(\varepsilon  > 0\) ，存在足够大的整数 \(N\) 使得

\[
\begin{Vmatrix}{{x}_{m} - {x}_{n}}\end{Vmatrix} \leq  \varepsilon \;\text{ whenever }m,n \geq  N.
\]
...
图2.1.1. 例2.3所述的 \({\mathbb{R}}^{2}\) 中单位球，其范数分别为 \(\|  \cdot  {\| }_{1}\) (左)、 \(\|  \cdot  {\| }_{2}\) (中)和 \(\|  \cdot  {\| }_{\infty }\) (右).

\section*{2.1.1. 赋范空间与Banach空间的例子.}

例2.2. 具有欧氏范数的有限维空间 \({\mathbb{R}}^{n} = \left\{  {x = \left( {{x}_{1},\ldots ,{x}_{n}}\right) ,{x}_{i}}\right. \; \left. { \in  \mathbb{R}}\right\}\)
...
例2.3. 在空间 \({\mathbb{R}}^{n}\) 上可考虑如下替代范数
...
例2.4. 对任意闭有界区间 \(\left\lbrack  {a,b}\right\rbrack\) ，所有连续函数 \(f : \left\lbrack  {a,b}\right\rbrack   \mapsto  \mathbb{R}\) 构成的空间 \({\mathcal{C}}^{0}\left( \left\lbrack  {a,b}\right\rbrack  \right)\) (赋予范数
...
例2.5. 设 \(\Omega\) 为 \({\mathbb{R}}^{n}\) 的开子集.对每个 \(1 \leq  p < \infty\) ，考虑所有勒贝格可测函数 \(f : \Omega  \mapsto  \mathbb{R}\) 构成的空间 \({\mathbf{L}}^{p}\left( \Omega \right)\) ，满足 \({\int }_{\Omega }{\left| f\left( x\right) \right| }^{p}{dx} < \infty\) .此空间在范数
...
例2.6. 对固定 \(p \geq  1\) ，考虑所有实数序列构成的空间，其 \(p\) 次幂可求和:
...
例2.7. 所有有界实数序列构成的空间 \({\ell }^{\infty }\) (赋予范数
...
注2.8. 在空间 \({\ell }^{p},1 \leq  p < \infty\) 中，考虑单位向量族(2.9)
...
注 2.9．在实数域或复数域上，绝对值 \(\left| \alpha \right|\) 刻画了数 \(\alpha\) 的大小.类似地，在向量空间 \(X\) 上，范数 \(\| f\|\) 可视为刻画元素 \(f \in  X\) 的大小.然而我们注意到，可在同一函数空间上采用不同的范数(从而得到不同的距离)，进而获得截然不同的收敛结果.下一例将对此加以说明.
...
定理 2.11(有界算子的连续性)．线性算子 \(\Lambda  : X \mapsto  Y\) 有界当且仅当它连续.
...
我们断言 \(V = X\) ，从而 \(X\) 本身是有限维的.否则，可找到一点 \(x \in  X \smallsetminus  V\) .令 \(\rho  \doteq  d\left( {x,V}\right)  \doteq  \mathop{\inf }\limits_{{y \in  V}}\| y - x\|\) .注意到 \(\rho  > 0\) ，因为 \(V\) 是闭的.因此存在一点 \(v \in  V\) ，使得
...
35. 设 \(Y\) 为Banach空间 \(X\) 的一个闭子空间，且假设 \({x}_{0} \in  X \smallsetminus  Y\) .证明存在一个有界线性泛函 \(\varphi  \in  {X}^{ * }\) ，使得 \(\| \varphi \|  \leq  1\) 且
