...
\[
{m}_{n}\left( \mathop{\bigcup }\limits_{{k = 1}}^{\infty }\right)  = \sum_{{k = 1}}^{\infty }{m}_{n}\left( {A}_{k}\right) \;\text{ (countable additivity). }
\]
...
A.3.2 Lebesgue积分.为定义Lebesgue积分，需从一类特殊函数出发.集合 $A$ 的特征函数为
...
\[
{\int }_{{\mathbb{R}}^{n}}{gdx} \doteq  \sum_{{i = 1}}^{N}{\mathrm{c}}_{i}{m}_{n}\left( {A}_{i}\right)
\]
...
\| f{\| }_{{L}^{p}\left( U\right) } \doteq  {\left( {\int }_{U}{\left| f\right| }^{p}dx\right) }^{1/p} < \infty .
\end{equation}
...
\[
\sum_{{i = 1}}^{n}\left( {{b}_{i} - {a}_{i}}\right)  < \delta \;\text{ implies }\;\sum_{{i = 1}}^{n}\left| {F\left( {b}_{i}\right)  - F\left( {a}_{i}\right) }\right|  < \varepsilon .
\]
...
\[
F\left( x\right)  = F\left( a\right)  + {\int }_{\left(  a,x\right)  }f\left( x\right) {dx}\;\forall\,x \in  \left(  {a,b}\right)  .
\]
...
\begin{itemize}\relax
\item\relax 积分函数 $F\left( y\right)  \doteq  {\int }_{X}{f}^{y}\left( x\right) {dx}$ 属于 ${L}^{1}\left( Y\right)$ ；
\end{itemize}
...
\[
{\iint }_{X \times  Y}f\left( {x,y}\right) {dxdy} = {\int }_{Y}\left(  {{\int }_{X}{f}^{y}\left( x\right) {dx}}\right)  {dy} = {\int }_{X}\left(  {{\int }_{Y}{f}^{x}\left( y\right) {dy}}\right)  {dx}.
\]
\end{theorem}
...
\[
{g}_{k}\left( t\right)  \rightarrow  f\left( t\right) \;\text{ for a.e. }t \in  \left(  {0,T}\right)  .
\]
...
强可测函数 $f : \left(  {0,T}\right)   \mapsto  X$ 可求和当且仅当标量函数 $t \mapsto  \| f\left( t\right) \|$ 可求和.在
...
\begin{equation}\tag{A.15}\label{eq:app-A-15}
...
2. 由Lebesgue微分定理，对几乎处处的 $x \in  \Omega$ ，有
...
\[
\left| {{f}_{\varepsilon }\left( x\right)  - f\left( x\right) }\right|  = \left| {{\int }_{B\left( {x,\varepsilon }\right) }{J}_{\varepsilon }\left( {x - y}\right) \left(  {f\left( y\right)  - f\left( x\right) }\right)  {dy}}\right|
\]
...
4. 为证明 (iv)，假设 $1 \leq  p < \infty$ 且 $f \in  {L}_{\text{ loc }}^{p}\left( \Omega \right)$ .设 $K \subset  \Omega$ 为一紧子集，并选取 $\rho  > 0$ ，使得 \eqref{eq:app-A-17} 中的紧邻域 ${K}_{\rho }$ 仍包含于 $\Omega$ 内.我们声称:对任意 $0 < \varepsilon  < \rho$ ，有
...
\[
x,y \in  \Omega ,\theta  \in  \left(  {0,1}\right)  \;\text{ implies }\;{\theta x} + \left( {1 - \theta }\right) y \in  \Omega .
\]
...
{\int }_{\Omega }\left| {fg}\right| {dx} \leq  \| f{\| }_{{L}^{p}\left( \Omega \right) }\| g{\| }_{{L}^{q}\left( \Omega \right) }.
\end{equation}

事实上，在 $\| f{\| }_{{L}^{p}\left( \Omega \right) } = \| g{\| }_{{L}^{q}\left( \Omega \right) } = 1$ 的特殊情形下，杨不等式给出

\[
{\int }_{\Omega }\left| {fg}\right| {dx} \leq  \frac{1}{p}{\int }_{\Omega }{\left| f\right| }^{p}{dx} + \frac{1}{q}{\int }_{\Omega }{\left| g\right| }^{q}{dx} = \frac{1}{p} + \frac{1}{q} = 1 = \| f{\| }_{{L}^{p}}\| g{\| }_{{L}^{q}}.
\]
...
{\int }_{\Omega }\left| {{f}_{1}{f}_{2}\cdots {f}_{m}}\right| {dx} \leq  \mathop{\prod }\limits_{{k = 1}}^{m}{\norm{f}_{k}}_{{L}^{{p}_{k}}\left( \Omega \right) }.
\end{equation}
...
\[
{\norm{f} + g}_{{L}^{p}\left( \Omega \right) }^{p} = {\int }_{\Omega }{\left| f + g\right| }^{p}{dx} = {\int }_{\Omega }{\left| f + g\right| }^{p - 1}\left( {\left| f\right|  + \left| g\right| }\right) {dx}
\]

\[
\leq  {\left( {\int }_{\Omega }{\left| f + g\right| }^{\left( {p - 1}\right)  \cdot  \frac{p}{p - 1}}dx\right) }^{\frac{p - 1}{p}}\left(  {{\left( {\int }_{\Omega }{\left| f\right| }^{p}dx\right) }^{\frac{1}{p}} + {\left( {\int }_{\Omega }{\left| g\right| }^{p}dx\right) }^{\frac{1}{p}}}\right)
\]
...
\[
\frac{1}{r} = \frac{\theta }{p} + \frac{1 - \theta }{q}\;\text{ for some }\theta  \in  \left(  {0,1}\right)  .
\]
...
(ii) 存在函数 $g \in  {L}^{1}\left( \mathbb{R}\right)$ ，使得其导数 ${f}_{n}^{\prime }$ 对每个 $n \geq  1$ 及几乎处处 $x \in  \mathbb{R}.$ 满足 $\left| {{f}_{n}^{\prime }\left( x\right) }\right|  \leq  g\left( x\right)$ .
