...
\[
{\left( f,{\left( {L}_{\gamma }^{ * }\right) }^{-1}g\right) }_{{L}^{2}} = {B}_{\gamma }\left(  {{L}_{\gamma }^{-1}f,{\left( {L}_{\gamma }^{-1}\right) }^{ * }g}\right)   = {\left( {L}_{\gamma }^{-1}f,g\right) }_{{L}^{2}}.
\]
\end{proof}
...
\[
B\left(  {u,v}\right)   = {B}_{\gamma }\left(  {u,v}\right)   - \gamma {\left( u,v\right) }_{{L}^{2}} = {\left( f + \gamma u,v\right) }_{{L}^{2}} - \gamma {\left( u,v\right) }_{{L}^{2}} = {\left( f,v\right) }_{{L}^{2}}.
\]
\end{proof}

为应用Fredholm理论(定理6.1)，我们除\eqref{eq:linpde-9-2}外还考虑齐次问题
\begin{equation}\tag{9.44}\label{eq:linpde-9-44}
\left\{  \begin{aligned} {Lu} &  = 0, & & x \in  \Omega , \\  u &  = 0, & & x \in  \partial \Omega , \end{aligned}\right.
\end{equation}

以及伴随问题
\begin{equation}\tag{9.45}\label{eq:linpde-9-45}
...
{\setjot[-2]\begin{gather*}
    ...
    v \equiv  0\;\text{是}\;\eqref{eq:linpde-9-45}\;\text{的唯一解. }
\end{gather*}}
\end{proof}
...
\begin{cases} {u}_{t} + {Lu} = 0, &  t > 0,x \in  \Omega , \\  u\left( {t,x}\right)  = 0, & t > 0,x \in  \partial \Omega , \\  u\left( {0,x}\right) = g\left( x\right) , & x \in  \Omega . \end{cases}
\end{equation}
...
\noindent 2. 若\eqref{eq:linpde-9-52}成立，则由 Lax-Milgram 定理，对任意 $f \in  {L}^{2}\left( \Omega \right)$ ，存在唯一 $u \in  {H}_{0}^{1}\left( \Omega \right)$，使得
\[
B\left(  {u,v}\right)   = {\left( f,v\right) }_{{L}^{2}},\;\forall\,v \in  {H}_{0}^{1}\left( \Omega \right) .
\]
...
设 $L$ 为(9.20)式中的算子.则对每个 $g \in  {L}^{2}\left( \Omega \right)$ ，公式\eqref{eq:linpde-9-57}定义了一个从 $\left(  {0,\infty \left(  \right. }\right.$ 到 ${L}^{2}\left( \Omega \right)$ 的映射 $t \mapsto  {u}_{t} = {S}_{t}g$ .该映射关于 $t \in  \left(  {0,\infty ) }\right.$ 连续，且关于 $t > 0$ 连续可微.此外，对每个 $t > 0$ 均有 $u\left( t\right)  \in  \operatorname{Dom}\left( L\right)  \subseteq  {H}_{0}^{1}\left( \Omega \right)$ ，且
...
3．对级数\eqref{eq:linpde-9-57}逐项求导，并注意到导数级数亦收敛，即可得到\eqref{eq:linpde-9-58}式.
\end{proof}
...
\[
u\left( {t,x}\right)  = \sum_{{k = 1}}^{\infty }\frac{2}{\pi }{e}^{-{k}^{2}t}\left( {{\int }_{0}^{\pi }g\left( y\right) \sin {kydy}}\right) \sin {kx}.
\]
\end{instance}
...
根据定理\ref{thm:linpde-9-9}，空间 ${L}^{2}\left( \Omega \right)$ 存在一组由 $L$ 的特征函数构成的规范正交基 $\left\{  {{\phi }_{k};k \geq  1}\right\}$ ，使得
...
(1) 初值条件与边界条件均被满足.考虑任意初值数据 $\begin{pmatrix}f \\ g\end{pmatrix}  \in  X = {H}_{0}^{1}\left( \Omega \right)  \times  {L}^{2}\left( \Omega \right)$ .由\eqref{eq:linpde-9-84}中映射的连续性可知
...
(2) 双曲方程在分布意义下成立.若函数 $f$ 与 $g$ 均为特征函数 ${\phi }_{k}$ 的有限线性组合，则相应轨迹\eqref{eq:linpde-9-84}是从 $\mathbb{R}$ 到 $X$ 的连续可微映射，且在所有时刻 $t > 0$ 满足\eqref{eq:linpde-9-74}.
...
\[
\iint \left\{  {u{\varphi }_{tt} + \sum_{{i,j = 1}}^{n}{a}^{ij}\left( x\right) {u}_{{x}_{i}}{\varphi }_{{x}_{j}}}\right\}  {\d x} = 0.
\]
...
(ii) 给定 $f \in  {L}^{2}\left( \Omega \right)$ ，证明存在唯一的 $u \in  X$ ，使得
...
(i) 验证 ${\phi }_{m,n} \in  {H}_{0}^{1}\left( Q\right)$ .此外，证明可数函数集 $\left\{  {{\phi }_{m,n};m,n \geq  1}\right\}$ 构成 ${L}^{2}\left( Q\right)$ 的一组标准正交基，且均由椭圆算子 ${Lu} =  - {\Delta u}$ 的特征函数组成.计算相应的特征值 ${\mu }_{m,n}$ .
...
\begin{rbhw}
(伽辽金逼近)设 $\Omega  \subset  {\mathbb{R}}^{n}$ 为有界开集.设 $\left\{  {{\varphi }_{k};k \geq  1}\right\}  ,{\varphi }_{k} \in  {H}_{0}^{1}\left( \Omega \right)$ 是 ${L}^{2}\left( \Omega \right)$ 的一组由拉普拉斯算子 $\Delta$ 的特征函数构成的标准正交基.
...
(ii) 令 $m \rightarrow  \infty$ ，证明序列 ${u}_{m}$ 在 ${H}_{0}^{1}\left( \Omega \right)$ 中一致有界，故存在弱收敛子列，记为 ${u}_{{m}_{j}} \rightharpoonup  u$ .证明 $u$ 是方程\eqref{eq:linpde-9-2}的弱解.由唯一性可知，整个序列收敛:当 $m \rightarrow  \infty$ 时， ${u}_{m} \rightharpoonup  u$ .
...
\[
{\varphi }_{1}\left( x\right)  = \left\{  \begin{matrix} x & \text{ if }x \in  \left(  {0,1}\right)  , \\  2 - x & \text{ if }x \in  \left(  {1,2}\right)  , \\  0 & \text{ if }x \in  \left(  {2,3}\right)  , \end{matrix}\right. \;{\varphi }_{2}\left( x\right)  = \left\{  \begin{matrix} 0 & \text{ if }x \in  \left(  {0,1}\right)  , \\  x - 1 & \text{ if }x \in  \left(  {1,2}\right)  , \\  3 - x & \text{ if }x \in  \left(  {2,3}\right)  . \end{matrix}\right.
\]
...
{u}_{tt} = 2{u}_{xx} + y{u}_{xy} + 3{u}_{yy} + \frac{1}{2}{u}_{x},\;\text{ on }\Omega  \times  \left(  {0,T}\right)
\end{equation}

\[
u = 0,\;\text{ on }\partial \Omega  \times  \left(  {0,T}\right)  .
\]

(i) 将方程\eqref{eq:linpde-9-89}写成形式 ${u}_{tt} + {Lu} = 0$ ，并证明算子 $L$ 在区域 $\Omega$ 上是一致椭圆的.
...
(i) 证明\eqref{eq:linpde-9-22}中的双线性泛函 $B$ 是 ${H}_{0}^{1}\left( \Omega \right)$ 上的一个内积.相应的范数
...
(ii) 记 ${H}_{\diamond }$ 为赋予该等价范数的Hilbert空间 ${H}_{0}^{1}$ .仿照引理\ref{lem:linpde-9-7}的证明，利用下图构造\eqref{eq:linpde-9-19}的解 $u = {\iota }^{ * }f$ :
...
\begin{rbhw}
设 $\Omega  \subset  {\mathbb{R}}^{N}$ 是一个有界开集.在引理\ref{lem:linpde-9-19}的同一设定下，证明映射 $\left. {t \mapsto  u\left( t\right) \text{ is }{\mathcal{C}}^{\infty }\text{ from }( 0,T}\right)$ 到 ${L}^{2}\left( \Omega \right)$ .

\end{rbhw}

\begin{rbhw}
(Neumann问题)设 $\Omega$ 是一个具有光滑边界 $\partial \Omega$ 的有界连通开集.依定义，函数 $u \in  {H}^{1}\left( \Omega \right)$ 是Neumann问题¹ 的弱解:
...
给定 $f \in  {L}^{2}\left( \Omega \right)$ ，证明边值问题\eqref{eq:linpde-9-92}存在唯一弱解.提示:证明由\eqref{eq:linpde-9-93}左端定义的、定义在 ${H}_{0}^{2}\left( \Omega \right)$ 上的双线性型 $B\left(  {u,v}\right)$ 在 ${H}_{0}^{2}\left( \Omega \right)$ 上严格正定.
