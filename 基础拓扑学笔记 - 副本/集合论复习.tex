...
\begin{definition}{幂集}
    设$X$是一个集合，$X$的所有子集组成的集合称为$X$的\textbf{幂集}，记作$2^X$.
\end{definition}

这个记号的一个合理解释是，若一个集合有$X$个元素，那么它有$2^X$个子集.

\begin{definition}{集合的卡氏积}
    设$X_1,X_2,\cdots,X_n$为集合，则它们的\textbf{卡氏积}定义为$X_1\times X_2\times\cdots\times X_n := \big\{(x_1,x_2,\cdots,x_n)|x_i\in X_i\big\}$.
\end{definition}
...
\begin{definition}{映射}
    设$X,Y$是两个集合，则$X,Y$之间的\textbf{映射}$f:X\to Y$将$X$中的每个元素$x$\textbf{唯一地}对应到$Y$中一个元素$y$.其中$X$称为\textbf{定义域}，$Y$称为\textbf{值域}.
\end{definition}
...
\end{definition}
从元素的角度来看，从一个集合的所有等价类中各选取一个代表元组成的集合就是它的商集.\par
$\alpha \beta \gamma \delta \epsilon \varepsilon \zeta \eta \theta \vartheta \iota \kappa \lambda \mu \nu \xi o \pi \varpi \rho \varrho \sigma \varsigma \tau \upsilon \phi \varphi \chi \psi \omega$ \par
$A B \Gamma \varGamma \Delta \varDelta E Z H \Theta \varTheta I K \Lambda \varLambda M N \Xi \varXi O \Pi \varPi P \Sigma \varSigma T \Upsilon \varUpsilon \Phi \varPhi X \Psi \Psi \varPsi \Omega \varOmega\sum\prod\int\partial $
