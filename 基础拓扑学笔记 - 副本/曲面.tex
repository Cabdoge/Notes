...
\begin{definition}[边界]
    设 $S$ 是一个曲面，若 $x \in S$ 有一个邻域同胚于 $\mathbb{E}^2_+$ 且该同胚将 $x$ 映为 $0 \in \mathbb{E}^2_+$，则称 $x$ 为 $S$ 的一个\textbf{边界点}.
    $S$ 的边界点的集合称为 $S$ 的\textbf{边界}，记为 $\partial S$.\par
    若紧致曲面 $S$ 满足 $\partial S = \varnothing$，则称其为\textbf{闭曲面}.
\end{definition}
依照上述定义，球面、环面、克莱因瓶和射影平面均为闭曲面.

\begin{definition}[曲面定向]
    若曲面 $S$ 包含一个嵌入的莫比乌斯带，则称 $S$ 是\textbf{不可定向的}.否则称其为\textbf{可定向的}.
\end{definition}
\begin{figure}[h]
    \centering
...
\begin{definition}
    设 $S_1, S_2$ 为两个曲面.$S_1$ 和 $S_2$ 的\textbf{连通和}，记作$S_1 \# S_2$，是通过从每个曲面中移除一个开圆盘，并将两个圆周边界粘合在一起而得到的.
\end{definition}
\begin{instance}
    球面$\#$环面 $\cong$ 环面.\par
    \begin{center}
...
\begin{instance}
    ...
    \[ \mathbb{R}P^2 - D_1^\circ \cup \mathbb{R}P^2 - D_2^\circ = \text{莫比乌斯带} \cup \text{莫比乌斯带} \cong \text{克莱因瓶} \]
    $\mathbb{R}P^2 \# \mathbb{R}P^2 \# \dots \# \mathbb{R}P^2$: $mP^2$, $m \ge 1$，不可定向.
    $m$ 称为 $mP^2$ 的\textbf{交叉帽数}.
\end{instance}

\begin{instance}
    $T^2 \# \mathbb{R}P^2$ 是一个不可定向曲面.
\end{instance}

\section{闭曲面的分类}

\begin{theorem}[闭曲面分类定理]
    每个闭曲面同胚于 $S^2$，$nT^2$，$mP^2$ ($n, m \ge 1$) 中的恰好一个.
\end{theorem}

\begin{definition}
    曲面 $S$ 中的一个\textbf{三角形}是指 $S$ 的一个子空间 $T$ 以及一个同胚 $h: \Delta \to T$，其中 $\Delta \subset \mathbb{E}^2$ 是一个闭三角形区域.
    若 $v$ 是 $\Delta$ 的顶点，则 $h(v)$ 是 $T$ 的顶点.
    若 $e$ 是 $\Delta$ 的边，则 $h(e)$ 是 $T$ 的边.
\end{definition}

\begin{definition}
    闭曲面 $S$ 的一个\textbf{三角剖分}是 $S$ 中有限个三角形的集合，它们覆盖 $S$，且满足：任意两个三角形要么不相交，要么相交于两个三角形的一个公共顶点，要么相交于两个三角形的一条公共边.
\end{definition}
...
\begin{definition}
    设 $S$ 是一个曲面，$T$ 是 $S$ 的一个三角剖分.$S$ 的\textbf{欧拉示性数}定义为
    \[ \chi(S) = V - E + F \]
...
\begin{instance}
    \begin{itemize}
        \item $\chi(T^2) = 9 - 27 + 18 = 0$
        \item $\chi(K) = 9 - 27 + 18 = 0$
        \item $\chi(\mathbb{R}P^2) = 6 - 15 + 10 = 1$
        \item $\chi(S^2) = 4 - 6 + 4 = 2$
    \end{itemize}
\end{instance}
...
\begin{proof}
    ...
    在进行连通和操作时，我们移除两个面（每个曲面一个），并将边界识别.
    \[ \chi(S_1 \# S_2) = (V_1 + V_2 - 3) - (E_1 + E_2 - 3) + (F_1 + F_2 - 2) \]
    \[ = (V_1 - E_1 + F_1) + (V_2 - E_2 + F_2) - 2 = \chi(S_1) + \chi(S_2) - 2 \]
\end{proof}

\begin{instance}
    \[ \chi(nT^2) = 2 - 2n, \quad \chi(mP^2) = 2 - m \]
\end{instance}
...
\begin{definition}
    设 $S$ 是一个闭曲面.
    \begin{itemize}
        \item $\tau$ 是一个具有偶数条边的多边形.
        \item $\varphi$ 是将 $\tau$ 的边成对粘合的映射.
    \end{itemize}
    $\tau/\varphi$ 是一个闭曲面.若 $\tau/\varphi \cong S$，则称 $(\tau, \varphi)$ 为 $S$ 的一个\textbf{多边形表示}.
\end{definition}
...
\begin{instance}
    $3T^2$: $a_1 b_1 a_1^{-1} b_1^{-1} a_2 b_2 a_2^{-1} b_2^{-1} a_3 b_3 a_3^{-1} b_3^{-1}$.
    \begin{center}
        \includegraphics[width=0.8\textwidth]{example-image}
    \end{center}
\end{instance}
...
\begin{instance}
    \begin{itemize}
        \item $\mathbb{R}P^2$: $a^2$
        \item $2P^2$: $a_1^2 a_2^2$
        \item $3P^2$: $a_1^2 a_2^2 a_3^2$
        \item $mP^2$: $a_1^2 a_2^2 \dots a_m^2$
    \end{itemize}
    \begin{center}
        \includegraphics[width=0.8\textwidth]{example-image}
    \end{center}
...
\textbf{反向对}：$\uparrow \dots \uparrow$
\textbf{同向对}：$\uparrow \dots \downarrow$
...
1.1 $P$ 中只有 1 个顶点，则只有 $a \dots a^{-1}$ 或 $a \dots a$.
作手术 A，消灭 $P$.
\begin{center}
    \includegraphics[width=0.8\textwidth]{example-image}
\end{center}
...
\begin{center}
    \includegraphics[width=0.8\textwidth]{example-image}
\end{center}
$a$ 是反向对，由引理，另一反向对 $b$ 与 $a$ 相间排列.利用手术 B，消去 $a$ 和 $b$，增加相间排列并且相邻的反向对 $a_1$ 和 $b_1$.
...
2.2.2 若有反向对.设 $a$ 是反向对.
由引理，边对 $b$ 与 $a$ 相间排列.因同向对都相邻，$b$ 是反向对.
\begin{center}
    \includegraphics[width=0.8\textwidth]{example-image}
\end{center}
利用手术 B，消灭 $a, b$，增加同向对 $a_1, b_1$.
...
\begin{definition}
    ...
    \begin{enumerate}
        \item $X$ 是 Hausdorff 空间.
        \item $X$ 是第二可数的.
        \item 对任意 $x \in X$，存在 $x$ 的一个邻域同胚于 $\mathbb{E}^n$ 或 $\mathbb{E}^n_+$，该同胚将 $x$ 映为 $0 \in \mathbb{E}^n$ 或 $0 \in \mathbb{E}^n_+$.
    \end{enumerate}
    则称 $X$ 为一个 \textbf{$n$ 维流形}.
