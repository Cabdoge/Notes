...
\begin{definition}{曲面上的三角形}
    设$S$是一个曲面，$S$中的一个\textbf{三角形}是指$S$ 的一个子空间 $T$ 以及一个同胚 $h: \Delta \to T$，其中 $\Delta \subset \mathbb{E}^2$ 是一个闭三角形区域，满足
    若 $v$ 是 $\Delta$ 的顶点，则 $h(v)$ 是 $T$ 的顶点.
    若 $e$ 是 $\Delta$ 的边，则 $h(e)$ 是 $T$ 的边.
\end{definition}

\begin{definition}
    闭曲面 $S$ 的一个\textbf{三角剖分}是 $S$ 中有限个覆盖 $S$的三角形的集合，且满足：任意两个三角形要么不相交，要么相交于两个三角形的一个公共顶点，要么相交于两个三角形的一条公共边.
\end{definition}
...
\begin{definition}{欧拉示性数}
    $S$ 是一个曲面，$T$ 是 $S$ 的一个三角剖分，则$S$ 的\textbf{欧拉示性数}定义为
    \[ \chi(S) = V - E + F \]
...
\textbf{反向对}：$\vcenter{\hbox{\includegraphics[width=0.1\textwidth]{fig/9.a-1.png}}}$
\textbf{同向对}：$\vcenter{\hbox{\includegraphics[width=0.1\textwidth]{fig/9.a-2.png}}}$
...
\textbf{1.1}\; $P$ 中只有 1 个顶点，则只有$\vcenter{\hbox{\includegraphics[width=0.1\textwidth]{fig/9.a.png}}}$或$\vcenter{\hbox{\includegraphics[width=0.1\textwidth]{fig/9.b.png}}}$.
作手术 A，消灭 $P$.
...
\end{lemma}
\begin{proof}
    如果反向对相邻$\vcenter{\hbox{\includegraphics[width=0.1\textwidth]{fig/9.c.png}}}$，则顶点类数 $>1$，矛盾.\par
