...
\section{正则空间和正规空间}
\begin{definition}[正则空间][def:5.1]
\index{z@正则空间}
    一个拓扑空间 $X$ 称为\colorbf{正则的}, 如果
    \begin{enumerate}
        \item[(i)] $X$ 中的单点集是闭集;
...
\begin{definition}[正规空间][def:5.2]
\index{z@正规空间}
    一个拓扑空间 $X$ 称为\colorbf{正规的}, 如果
    \begin{enumerate}
        \item[(i)] $X$ 中的单点集是闭集;
...
\section{可数性}
\begin{definition}[可数邻域基][def:5.3]
    设$X$是一个拓扑空间，$x\in X$，如果存在$x$的一个可数邻域族$\{U_n\}_{n \in \mathbb{Z}^+}$，使得$x$的任何邻域 $W$都包含其中一个 $U_n$, 即 $x \in U_n \subset W$，则称在点$x$处一个\colorbf{可数邻域基}.
\end{definition}
\vspace{-1.5 em}
\begin{figure}[h]
    \centering
...
\begin{definition}[第一可数][def:5.4]
\index{d@第一可数}
    如果 $X$ 在其每一点都有可数邻域基, 则称 $X$ 是\colorbf{第一可数的}，记为$C_1$空间.
\end{definition}
...
\begin{definition}[第二可数][def:5.5]
\index{d@第二可数}
    设$X$是一个拓扑空间，若$X$有一个可数基, 则称$X$是\colorbf{第二可数的}, 记为$C_2$空间.
\end{definition}
...
\section{两个定理}
\begin{theorem}[Urysohn度量化定理][thm:5.1]
...
\begin{theorem}[Tietze扩张定理][thm:5.2]
