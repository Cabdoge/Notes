...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \end{tikzcd}
        \]
        我们有 $h^{-1} \circ f = q$.因为 $q$ 是连续的，而 $f$ 是商映射（根据抽象定义），再次使用引理 \ref{lem:quotient_map_char}，可知 $h^{-1}$ 是连续的.
    \end{itemize}
    因此，$h$ 是一个同胚.
\end{proof}
...
\begin{instance}
    ...
    可以验证$f$是连续满射.由于$D^n$ 是紧的，$S^n$ 是Hausdorff的，由命题\ref{pro:8.3}可知$f$ 是一个商映射.\par
...
\begin{definition}[射影空间]
    $n$维\textbf{射影空间}，记作 $\mathbb{P}^n$，有以下几种等价的构造方式：
    \begin{enumerate}[(i)]
