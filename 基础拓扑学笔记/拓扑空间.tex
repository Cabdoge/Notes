...
\begin{instance}
    考虑$\mbb{R}$上的一个拓扑基$\left\{\left[n,n+1\right)\right|n\in\mbb{Z}\}$生成的拓扑$\mcal{T}$，但在$\mcal{T}$中单点集$\{0\}$不是闭集.\par
...
\begin{definition}{Hausdorff空间}
    一个.扑空间$(X,\mcal{T})$是\textbf{Hausdorff}的，如果对$X$中任意两个不同的点$x,y$,都$\exists\, x,y$的邻域$U,V$且$U\cap V=\varnothing$.
\end{definition}
