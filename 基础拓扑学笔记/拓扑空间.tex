...
\begin{theorem}[][thm:2.1]
    $f:\mathbb{R}\to\mathbb{R}$是连续函数当且仅当任何开集的原像是开集.
\end{theorem}

关于开集，它有以下的性质.

\begin{proposition}[实数集上开集的性质][prop:2.1]
    \begin{enumerate}
...
\begin{definition}[拓扑空间][def:2.1]\index{t@拓扑空间}\index{k@开集}
    ...
    \end{enumerate}
    则称$\mathcal{T}$为$X$上的\colorbf{拓扑}，$(X,\mathcal{T})$为一个\colorbf{拓扑空间}，$\mathcal{T}$中的子集称为\colorbf{开集}.
\end{definition}

应用数学归纳法，其中的条件3还可以予以弱化.
\begin{proposition}[条件3的等价条件][prop:2.2]
    $\mathcal{T}$中有限个子集的交仍属于$\mathcal{T} \Leftrightarrow \mathcal{T}$中两个子集的交属于$\mathcal{T}$.
\end{proposition}

\begin{instance}
    (1)在实数集$\mathbb{R}$中，$\mathcal{T}:=\left\{\bigcup_\lambda{(a_\lambda,b_\lambda)}\left|(a_\lambda,b_\lambda)\text{是}\mathbb{R}\text{上的开区间}\right.\right\}$是拓扑空间.\par
    ...
    (3)$X$是一个非空集合，$\mathcal{T}_1=\{\varnothing,X\}$，被称为\bluebf{平凡拓扑}.$\mathcal{T}_2=2^X$，被称为\bluebf{离散拓扑}.\index{l@离散拓扑}\par
    ...
    \end{enumerate}
    因此$\mathcal{T}$是$\mathbb{R}$上的拓扑，称为\bluebf{有限补拓扑}，记作$\mathbb{R}_{fc}$.\index{y@有限补拓扑}
\end{instance}

由上面的例子可以看出，在同一个集合上可以定义不同的拓扑(至少有平凡拓扑和离散拓扑两种拓扑)，不同的拓扑之间可以用以下关系比较.

\begin{definition}[拓扑的精细与粗糙][def:2.2]\index{t@拓扑的精细与粗糙}
    设$\mcal{T}_1,\mcal{T}_2$是集合$X$上的两个拓扑，如果$\mcal{T}_1\subset \mcal{T}_2$，则称$\mcal{T}_2$比$\mcal{T}_1$ \colorbf{精细}，或称$\mcal{T}_1$比$\mcal{T}_2$ \colorbf{粗糙}.
\end{definition}
...
\begin{definition}[邻域][def:2.3]\index{l@邻域}
    设$X$是一个拓扑空间，$x\in X$，任意包含$x$的\uline{开集}$U$称为$x$的一个\colorbf{邻域}.
\end{definition}

\begin{theorem}[][thm:2.2]   
    设$A\subset X$，则$A$是开集当且仅当对任意$x\in A$，存在$x$的一个邻域$U$，使得$x\in U\subset A$.
\end{theorem}\par
\begin{proof}
    “$\Rightarrow$” 显然成立.\par
    \begin{wrapfigure}{r}{0.2\textwidth}
    \vspace{-3 em}
    \includegraphics[width=0.2\textwidth]{fig/2.1.1.png}
    \vspace{0 em}
    \end{wrapfigure}
    “$\Leftarrow$” 由于$\forall\, x \in A$，都存在$x$的一个邻域$U_x,\st x\in U_x\subset A$，所以$A=\bigcup_{x\in A}{U_x}$.\par
    由于$U_x$是$X$的一个开集，所以$A$是一个开集.    
\end{proof}

\section{拓扑基}
\begin{definition}[拓扑基][def:2.4]\index{t@拓扑基}
...
\end{figure}\par
上面两个条件中，第一个指出拓扑基可以覆盖整个集合$X$，第二个条件则确保了两个基元素的交集仍然可以用基元素来表示，这是保证拓扑基对有限交运算封闭的关键.\par

\begin{theorem}[拓扑基生成拓扑][thm:2.3]
\index{t@拓扑基生成拓扑}
    设$\mcal{B}$为$X$的拓扑基，$\mcal{T}=\{\mcal{B}\text{中任意多个子集的并}\}$，则$\mcal{T}$是$X$上的拓扑，称其为由拓扑基$\mcal{B}$生成的拓扑，$\mcal{B}$中的子集称为\colorbf{基元素}.
\end{theorem}
...
\begin{proposition}[给定拓扑的拓扑基][prop:2.3]
...
\begin{proof}
    \begin{enumerate}
        ...
        "$\supset$" $\forall\, U\in\mcal{T},\forall\, x\in U$，根据假设，$\exists\, V\in\mcal{C},\st x\in V\subset U$.由\cref{lem:2.2}，$U\subset \mcal{T}_1$，因此$\mcal{T}_1\supset \mcal{T}$.\par
        所以$\mcal{T}=\mcal{T}_1$. \qedhere
    \end{enumerate}
\end{proof}
...
\begin{wrapfigure}{r}{0.25\textwidth}
    ...
    \vspace{-5 em}
\end{wrapfigure}\par
(1)首先$(a_1,b_1)\times \cdots \times (a_n,b_n)$是$\mathbb{E}^n$上的开集.取$n=2$为例，如右图所示，取长方形中的任意一个点，都可以在长方体里找到一个包含这个点的圆，因此满足\cref{lem:2.2}中对开集的描述.\par
\vspace{3 em}
...
\begin{proof}
    \begin{enumerate}[(i)]
        ...
        \item 因为$U_1\cap U_2\in\mcal{T}$，所以$\left(U_1\cap A\right)\cap \left(U_2\cap A\right)=(U_1\cap U_2)\cap A\in\mcal{T}_A$. \qedhere
    \end{enumerate}
\end{proof}

这个拓扑的定义方式和限制映射有些类似，它被我们称为\bluebf{子空间拓扑}.

\begin{definition}[拓扑子空间][def:2.5]
\index{z@子空间}
    $(A,\mcal{T}_A)$称为$(X,\mcal{T})$的\colorbf{子空间}，$\mcal{T}_A$称为$A$上的\colorbf{子空间拓扑}，或称\colorbf{诱导拓扑}.
\end{definition}
...
\begin{proof}
    ...
    由\cref{lem:2.1}可知$\mcal{B}$为$X\times Y$上的一个拓扑基.
\end{proof}
\vspace{5 em}

\begin{definition}[乘积拓扑][def:2.6]
    $\mcal{B}$所生成的拓扑称为$X\times Y$上的\colorbf{乘积拓扑}，称$(X\times Y,\mcal{B})$为$X$和$Y$的\colorbf{乘积拓扑空间}.
\end{definition}
...
\begin{definition}[n维乘积拓扑][def:2.7]
...
\begin{definition}[投射][def:2.8]
    定义$\begin{aligned}[t]
        p_i: X_1\times X_2\times\cdots X_n &\to X_i \\
        (x_1,x_2,\cdots,x_n) &\mapsto x_i
    \end{aligned}$为$X_1\times X_2\times\cdots\times X_n$到$X_i$的\colorbf{投射}.
\end{definition}

\section{闭集}

\begin{definition}[闭集][def:2.9]\index{b@闭集}
    设$(X,\mcal{T})$是一个拓扑空间，$C\subset X$，如果$X-C$是开集，则称$C$为$X$中的\colorbf{闭集}.
\end{definition}
...
\begin{proposition}[闭集的性质][prop:2.5]
    设$X$是一个拓扑空间，则
    \begin{enumerate}
...
\begin{lemma}[子空间上的闭集][lem:2.8]\index{b@闭集!z@子空间上的闭集}
...
\begin{definition}[Hausdorff空间][def:2.10]\index{H@Hausdorff空间}
    一个拓扑空间$(X,\mcal{T})$是\colorbf{Hausdorff} 的，如果对$X$中任意两个不同的点$x,y$,都$\exists\, x,y$的邻域$U,V$且$U\cap V=\varnothing$.
\end{definition}
...
\begin{lemma}[Hausdorff性的必要条件][lem:2.9]
    如果$X$是Hausdorff的，则$X$中的任意单点集为闭集.
\end{lemma}\par
\wrapfix
\begin{proof}
    $\forall\, \{x\}\subset X$，只需证明其补集$X\setminus \{x\}$为开集.\par
    $\forall\, y\in X\setminus \{x\},y\neq x$，由于$X$是Hausdorff的，$\exists\, x,y$的邻域$U_x,U_y,\st U_x\cap U_y=\varnothing$，所以$y\in U_y\subset X-\{x\}$，因此$X-\{x\}$为开集.
\end{proof}
...
\begin{definition}[内部与闭包][def:2.11]
\index{n@内部}\index{b@闭包}
    设$X$是一个拓扑空间，$A\subset X$是一个子集，则$A$的\colorbf{内部}是指$A$中所有开集的并，记作$\mathring{A}$或$\mathrm{Int}(A)$.\par
    $A$的\colorbf{闭包}是指所有包含$A$的闭集的交，记作$\overline{A}$或$\mathrm{Cl}(A)$.\par
...
\begin{proposition}[内部与闭包的性质][prop:2.8]
...
\begin{proof}
    ...
    \quad\,“$\Leftarrow$” 因为$\mathring{A}$是开集，并且$A=\mathring{A}$，所以$A$是开集.
\end{proof}
...
\begin{definition}[稠密性和可分性][def:2.12]
    设$X$是一个拓扑空间，$A\subset X$，如果$\overline{A}=X$，则称$A$在$X$中是\colorbf{稠密}的.\par
    如果$X$中存在一个可数的稠密子集，则称$X$是\colorbf{可分}的. 
\end{definition}
...
\begin{proof}
    \begin{enumerate}[(i)]
        ...
        因此$x\in (U_1\cap U_2)\subset A\cap B$，其中$U_1\cap U_2$为开集，根据\cref{lem:2.10}，$x\in{(A\cap B)}^\circ$.\par
...
\begin{definition}[极限点][def:2.13]
\index{j@极限点}
    设$X$是一个拓扑空间，$A\subset X$，$x\in X$. 如果$x$的每个邻域都包含$A$中异于$x$的点，则称$x$是$A$的一个\colorbf{极限点}(或\colorbf{聚点}). $A$的所有极限点的集合记作$A'$.
\end{definition}
...
\begin{definition}[边界][def:2.14]
\index{b@边界}
    设$X$是一个拓扑空间，$A\subset X$，则$A$的\colorbf{边界}是指$\overline{A}-\mathring{A}$，记作$\partial A$.
\end{definition}
...
\begin{lemma}[][lem:2.11]
    设$X$是一个拓扑空间，$A\subset X$，$y\in X$，则$y\in\partial A\Leftrightarrow y$的任意邻域都与$A$和$X-A$相交.
\end{lemma}
\begin{proof}
    “$\Rightarrow$”\, $y\in\partial A=\overline{A}-\mathring{A}$.因此$y\in\overline{A}$，由\cref{lem:2.10}可知$y$的任意邻域都与$A$相交.\par
