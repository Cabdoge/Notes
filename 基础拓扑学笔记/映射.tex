...
\begin{definition}[连续映射][def:3.1]
\index{l@连续映射}
    设$X,Y$是拓扑空间，$f:X\to Y$是映射. 如果对$Y$中任意开集$V$，其原像$f^{-1}(V)$在$X$中是开集，则称$f$是\colorbf{连续的}.
\end{definition}
...
\begin{definition}[同胚][def:3.2]
\index{t@同胚}
    设 $X,Y$ 是拓扑空间，$f:X\to Y$ 是一个具有逆映射 $f^{-1}:Y\to X$ 的双射.如果 $f$ 和 $f^{-1}$ 都是连续的, 则称 $f$ 是一个\colorbf{同胚}.\par
    如果存在一个在 $X$ 和 $Y$ 之间的同胚, 则称 $X$ 和 $Y$ 是\colorbf{同胚的}, 记作 $X\cong Y$.
\end{definition}
...
\begin{definition}[开映射和闭映射][def:3.3]
    映射 $f:X\to Y$ 称为\colorbf{开映射} (相应地, \colorbf{闭映射})，如果 $f$ 将 $X$ 中的每个开集 (相应地, 闭集) 映射到 $Y$ 中的一个开集 (相应地, 闭集).
\end{definition}
...
最后我们引入如下的概念：
\begin{definition}[拓扑性质][def:3.4]
    一个拓扑空间的性质, 如果在同胚下保持不变, 则称之为一个\colorbf{拓扑性质}.
\end{definition}
因此，一个拓扑空间的Hausdorff性质是一个拓扑性质.\par

\section{嵌入}

\begin{definition}[嵌入][def:3.5]
\index{q@嵌入}
    设$X,Y$是拓扑空间，则$X$ 在 $Y$ 中的一个\colorbf{嵌入}是一个连续映射 $f:X\to Y$, 它将 $X$ 同胚地映射到子空间 $f(X)\subset Y$ 上.
\end{definition}
\begin{figure}[h]
    \centering
...
\begin{definition}[同痕][def:3.6]
    ...
    $F(x,0)=f_0(x)$, $F(x,1)=f_1(x)$, 并且对$\forall\, t\in[0,1]$, 映射 $f_t=F(\cdot,t):\setjot[-6]\begin{aligned}[t]
        S^1&\to\mb{E}^3\\
        x&\mapsto F(x,t)
    \end{aligned}$ 都是一个嵌入，那么称 $f_0$ 和 $f_1$ 是\colorbf{同痕的 (isotopic)}.
\end{definition}
...
\index{d@度量空间}
    ...
    \end{enumerate}
    则称$d$是$X$的一个\colorbf{度量}，$d(x,y)$称为$x$和$y$之间的\colorbf{距离}，$(X,d)$ 称为一个\colorbf{度量空间}.
\end{definition}
...
在度量空间上，可以自然地定义如下的拓扑：
\begin{definition}[度量拓扑][def:4.2]
\index{d@度量拓扑}
    $\forall\, x\in (X,d)$, 开球 $B(x,\varepsilon)=\{y\in X | d(x,y)<\varepsilon\}$.则$\mcal{B}=\{B(x,\varepsilon)|x\in X, \varepsilon>0\}$ 是 $X$ 上的一个拓扑基.由这个拓扑基生成的拓扑称为由度量 $d$ \colorbf{诱导的度量拓扑}.
\end{definition}
...
\begin{definition}[可度量化][def:4.3]
\index{k@可度量化}
    $X$ 是一个拓扑空间.如果在 $X$ 上存在一个度量 $d$ 诱导了 $X$ 上的拓扑，则称$X$ 是\colorbf{可度量化的}.
\end{definition}
