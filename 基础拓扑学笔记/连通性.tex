...
设
$$
    X=\big(\{0\}\times[-1,1]\big)\cup\big\{\, (x,\,\sin\tfrac{1}{x})\mid 0<x\le 1 \,\big\}\subset \mathbb{E}^2.
$$
记 $A=\{0\}\times[-1,1]$，$B=\{(x,\sin\tfrac{1}{x})\mid 0<x\le 1\}$.
\begin{remark}
...
\end{proposition}
\begin{proof}
    首先 $B$ 与区间 $(0,1]$ 同胚，故 $B$ 连通；且 $X=\overline{B}$（$A$ 正是 $B$ 的聚点集），由“连通集合的闭包仍连通”命题得 $X$ 连通.

    为证 $X$ 非道路连通，反设存在道路 $f:[0,1]\to X$，使 $f(0)=(0,0)\in A$ 且 $f(1)=(1,\sin 1)\in B$.注意 $A$ 在 $X$ 中是闭集，故 $f^{-1}(A)$ 为 $[0,1]$ 中闭集.令 $b=\max f^{-1}(A)$，则 $f(b)\in A$，且 $f((b,1])\subset B$.令 $h=f\circ g:[0,1]\to X$，其中 $g(t)=(1-t)b+t$，则 $h(0)=f(b)\in A$，$h(t)\in B$ 对 $t\in(0,1]$ 成立.

    声称：对每个 $n\in\mathbb{Z}_{>0}$，存在 $t_n\in(0,\tfrac{1}{n})$，使得 $h(t_n)=\big(x(t_n),\,y(t_n)\big)$ 满足 $y(t_n)=\sin\tfrac{1}{x(t_n)}=(-1)^n$.于是 $t_n\to 0$，序列 $h(t_n)=\big(x(t_n),(-1)^n\big)$ 在 $\mathbb{E}^2$ 中不收敛，违背 $h$ 的连续性.故不存在上述道路，$X$ 非道路连通.

    证明该声称：对给定 $n$，取整数 $k$ 使得
    $$
        0=x(0)<\frac{1}{\tfrac{\pi}{2}+(n+2k)\pi}<x\!\left(\tfrac{1}{n}\right).
    $$
    由 $x(t)$ 的连续性与介值定理，存在 $t_n\in(0,\tfrac{1}{n})$ 使
    $$
        x(t_n)=\frac{1}{\tfrac{\pi}{2}+(n+2k)\pi},\quad
        \Rightarrow\quad \sin\frac{1}{x(t_n)}=\sin\!\Big(\tfrac{\pi}{2}+n\pi\Big)=(-1)^n.
    $$
    声称得证.
\end{proof}

\begin{lemma}[极大性表述]
    若 $C$ 是 $X$ 的一个连通分支，且 $A\subset X$ 连通并满足 $C\subset A$，则 $A=C$.
\end{lemma}
\begin{proof}
    按定义，连通分支是极大连通子集.由于 $A$ 连通且包含 $C$，若 $A\ne C$ 则与 $C$ 的极大性矛盾，故 $A=C$.
\end{proof}

\section{局部道路连通}

\begin{definition}[局部道路连通]
    拓扑空间 $X$ 称为\textbf{局部道路连通}的，若对每个点 $x\in X$ 及其每个邻域 $U$，存在道路连通的开邻域 $V$，使 $x\in V\subset U$.
\end{definition}

\begin{remark}
    若 $X$ 局部道路连通，则每个点拥有道路连通基（即可以选取全由道路连通开集组成的局部基）.
\end{remark}

\begin{instance}
    $\mathbb{E}^n$ 的任一开子集是局部道路连通的：给定 $x$ 与开邻域 $U$，选小球 $B(x,\varepsilon)\subset U$，而 $B(x,\varepsilon)$ 是凸集因而道路连通.
\end{instance}

\begin{instance}
    令 $X=\{(x,y)\in\mathbb{E}^2\mid x\in\mathbb{Q}\text{ 或 }y=0\}$.$X$ 是道路连通的：任意点可先垂直/水平移动到 $x$ 轴，再沿 $x$ 轴连接.但 $X$ 不是局部道路连通的：点 $(0,y_0)$，$y_0\ne 0$ 的任一充分小邻域都会含有与 $x$ 轴不在同一条道路连通开集内的离散“竖线”碎片.
\end{instance}

\begin{instance}
    拓扑学家正弦曲线 $X$ 不是局部道路连通的：在 $(0,y)$ 附近的任何小邻域都包含 $A$ 与 $B$ 的“振荡”部分，无法找到包含 $(0,y)$ 的足够小道路连通开邻域.
\end{instance}
...
\end{proposition}
\begin{proof}
    将 $X$ 按道路连通分支分解为互不相交的道路连通开集族 $\{P_\lambda\}$（道路连通分支在局部道路连通空间中是开集）.若分支多于一个，则任选 $P_{\lambda_0}$，其补的并是开集，故 $P_{\lambda_0}$ 既开又闭且非空真子集，违背 $X$ 连通.故只有一个道路连通分支，$X$ 道路连通.
\end{proof}

\begin{corollary}
    若 $A\subset \mathbb{E}^n$ 是开且连通，则 $A$ 道路连通.
\end{corollary}

\subsection*{利用连通性质区分空间}

\begin{lemma}[限制仍为同胚]
    若 $f:X\to Y$ 是同胚，$A\subset X$，则限制 $f|_A:A\to f(A)$ 也是同胚.
\end{lemma}
\begin{proof}
    $f|_A$ 双射；连续性与开性由 $f$ 的连续开映射性质通过子空间拓扑直接继承：若 $U\subset A$ 在子空间拓扑开，则存在 $U'=U\cup (X\setminus A)$ 在 $X$ 中开，$f(U)=f(U')\cap f(A)$ 在 $f(A)$ 中开.逆映射同理.
\end{proof}

\begin{proposition}
    $\mathbb{E}^1\not\cong \mathbb{E}^n$（$n\ge 2$）.
\end{proposition}
\begin{proof}
    若存在同胚 $f: \mathbb{E}^1\to\mathbb{E}^n$，取一点 $p\in \mathbb{E}^1$.则 $\mathbb{E}^1\setminus\{p\}$ 不连通（分成两个开区间），而 $\mathbb{E}^n\setminus\{f(p)\}$ 连通（对 $n\ge 2$）.由限制同胚引理得到 $f|_{\mathbb{E}^1\setminus\{p\}}:\mathbb{E}^1\setminus\{p\}\to \mathbb{E}^n\setminus\{f(p)\}$ 同胚，这不可能，因为连通性为拓扑不变量.矛盾，故不存在.
\end{proof}
...
\end{proposition}
\begin{proof}
    若 $f:S^1\to S^n$ 为同胚，取点 $p\in S^1$.则 $S^1\setminus\{p\}$ 同胚于 $\mathbb{E}^1$ 不连通，而 $S^n\setminus\{f(p)\}$ 同胚于 $\mathbb{E}^n$ 连通（$n\ge 2$）.同样由限制同胚引理导出矛盾.
\end{proof}
