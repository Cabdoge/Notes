...
\vspace{-1 em}
\begin{definition}[连通空间][def:7.1]
\index{l@连通空间}
    设$X$是一个拓扑空间.如果存在非空、不相交的开集 $U$ 和 $V$，使得 $U \cup V = X$，则称$X$是\colorbf{不连通的}，$\{U, V\}$ 是$X$的一个\colorbf{分割}.否则，称$X$是\colorbf{连通的}.
\end{definition}
...
\begin{definition}[连通子集][def:7.2]
    设$X$是拓扑空间，$A \subset X$.如果$A$在子空间拓扑中是连通的，则称$A$在$X$中是\colorbf{连通的}（或$A$是$X$中的一个\colorbf{连通子集}）.
\end{definition}
...
\begin{lemma}[][lem:7.1]
    ...
    这样的 $\{U, V\}$ 称为$A$在$X$中的一个\colorbf{分割}.
\end{lemma}
\begin{figure}[h]
    \centering
...
\begin{lemma}[引理7.2的一般形式][lem:7.3]
...
\begin{proposition}[连通子集的闭包连通][prop:7.3]
...
\begin{proof}
    ...
    由\cref{lem:7.3}知 $A\subset U$ 或 $A\subset V$.不失一般性，不妨设 $A\subset V$，则 $A\cap U=\varnothing$.\par
...
\begin{lemma}[交集非空的连通族的并连通][lem:7.4]
...
\begin{theorem}[乘积空间的连通性][thm:7.3]
...
\begin{proof}
    ...
    记$C_y:= (X\times\{y\})\cup(\{x_0\}\times Y)$ 连通，由\cref{lem:7.4}，$C_y$连通.\par
    $X\times Y=\bigcup_{y\in Y} C_y$，且 $\bigcap_{y\in Y} C_y=\{x_0\}\times Y\neq\varnothing$.由\cref{lem:7.4}，$X\times Y$ 连通.
\end{proof}
\vspace{2 em}

这个定理的结论是自然的，并且应用数学归纳法可以将其推广到有限乘积拓扑空间的情形.

\begin{corollary}[有限乘积空间的连通性][cor:7.1]
...
\begin{definition}[连通分支][def:7.3]
    ...
    \end{enumerate}
    “$\sim_c$”的每一个等价类称为 $X$ 的一个\colorbf{连通分支}.
\end{definition}
\begin{proof}
...
\begin{proof}
    ...
    因为$\bigcap_{p\in C} C_p\supset\{x_0\}\neq\varnothing$，由\cref{lem:7.4}，$C=\bigcup_{p\in C} C_p$ 连通.
\end{proof}

\begin{lemma}[连通分支的极大性][lem:7.6]
...
\begin{definition}[道路][def:7.4]
\index{d@道路}
    在拓扑空间 $X$ 中，称从 $x$ 到 $y$ 的一条\colorbf{道路}为一个连续映射 $a:[0,1]\to X$，满足 $a(0)=x$，$a(1)=y$.
\end{definition}
\begin{figure}[h]
    \centering
...
\begin{definition}[道路连通][def:7.5]
\index{d@道路连通}
    在拓扑空间$X$中，若对任意 $x,y\in X$，都存在从 $x$ 到 $y$ 的道路，则称 $X$ 是\colorbf{道路连通}的.\par
    若 $A\subset X$ 在子空间拓扑中是道路连通的，则称 $A$ 是 $X$ 的一个\colorbf{道路连通子集}.
\end{definition}
...
\begin{proposition}[连续映射保持道路连通性][prop:7.4]
...
\begin{definition}[道路连通分支][def:7.6]
    ...
    \end{enumerate}
    “$\sim_p$”的每一个等价类称为 $X$ 的一个\colorbf{道路连通分支}.
\end{definition}
\begin{proof}
...
\begin{lemma}[道路连通分支是道路连通的][lem:7.8]
...
\begin{lemma}[道路连通分支的极大性][lem:7.9]
...
\begin{proposition}[道路连通必连通][prop:7.5]
    若拓扑空间 $X$ 道路连通，则 $X$ 连通.
\end{proposition}
\begin{proof}
...
\begin{definition}[局部道路连通][def:7.7]
\index{j@局部道路连通}
    设$X$是一个拓扑空间，若对$\forall\,x\in X$及其任意$x$的邻域$U$，存在$x$的道路连通的邻域$V$，使得$x\in V\subset U$，则称$X$为\colorbf{局部道路连通}.
\end{definition}
...
\begin{proposition}[连通且局部道路连通蕴含道路连通][prop:7.6]
...
\begin{corollary}[][cor:7.2]
    若 $A\subset \mathbb{E}^n$ 是开集且连通，则$A$道路连通.
\end{corollary}

\section{区分拓扑空间}

\begin{lemma}[限制仍为同胚][lem:7.10]
