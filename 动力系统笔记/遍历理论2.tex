...
\begin{proof}
    ...
    由于$A$是紧集，存在有限子覆盖$S^{-l_i}V_i,S^{-(l_i-1)V_i,\cdots,V_i},\cdots,S^{l_i}V_i(\forall\, 1\leq i\leq m)$.\par
    令$L=\max_{1\leq i\leq m}{l_i}$，则$\left\{S^{-L}V_i,S^{-(L-1)}V_i,\cdots,V_i,\cdots,S^{L}V_i\right\}$是$A$ 的一个开覆盖.\par
    \begin{itemize}
        \item $\forall\, x,y\in A,\exists\, l\in\{-L,-(L-1),\cdots,L\}$ ，使得$d\left(S^ly,x\right)<\varepsilon$.\par
    ...
    由上面的结论，$\forall\, x\in A,\exists\, l\in\{-L,\cdots,L\}$ ，使得$d \left(S^lx_0,x\right)<\varepsilon$.\par
