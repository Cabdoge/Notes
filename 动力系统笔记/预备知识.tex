...
\begin{definition}[$\sigma-\!\!$代数]
    ...
    \end{enumerate}
    则称$\mathcal{B}$是$X$的一个 \colorbf{$\sigma-\text{代数}$}.
\end{definition}

除了上述定义中满足的三个条件之外，还可以证明$\sigma-\text{代数}$满足以下两个性质.

\begin{property}[$\sigma-\text{代数}$]
    \begin{enumerate}
...
\begin{proof}
    \begin{enumerate}
        ...
        $\Rightarrow X\setminus\left(\bigcup_{i=1}^\infty{A_i^c}\right)=\bigcap_{i=1}^\infty{A_i}\in\mathcal{B}$.\qedhere
    \end{enumerate}
\end{proof}
...
\begin{definition}[概率测度]
    ...
    \end{enumerate}
    则称$m$为$(X,\mathcal{B})$上的一个\colorbf{概率测度}，$(X,\mathcal{B},m)$是一个\colorbf{概率测度空间}。
\end{definition}
...
但很多时候，$\sigma-\!\!$代数上的测度不容易计算，因此我们需要引入\bluebf{半代数}与\bluebf{代数}的概念.
\begin{definition}[半代数]\label{def:semi-algebra}
    ...
    \end{enumerate}
    称$\mathcal{S}$是$X$的一个 \colorbf{半代数}.
\end{definition}

\begin{instance}
    (1)取$X=[0,1]$，$\mathcal{S}=\{X\text{中所有区间}\}\cup\{\varnothing\}$.
    则$\mathcal{S}$是$X$的一个半代数.\par
    \begin{tikzpicture}[x=8cm,y=1cm,scale=0.85,>=Latex,
  tick/.style={thick},
...
\begin{definition}[代数]
    ...
    \end{enumerate}
    称$\mathcal{A}$是$X$的一个 \colorbf{代数}.
\end{definition}
...
\begin{definition}[最小代数]
    ...
    则$\mathcal{A}(\mathcal{S})$是一个代数，称为包含$\mathcal{S}$的 \colorbf{最小代数}.
\end{definition}
下面我们证明$\mathcal{A}(\mathcal{S})$是一个代数.
\begin{proof}
    (1)$\varnothing\in\mathcal{A}(\mathcal{S})$\par
    (2)$A,B\in\mathcal{A}(\mathcal{S})\Rightarrow A,B\in\mathcal{A}(\mathcal{A}\supseteq\mathcal{S}) \Rightarrow A\cap B\in\mathcal{A}\Rightarrow A\cap B\in\mathcal{A}(\mathcal{S})$\par
    (3)$A\in\mathcal{A}(\mathcal{S})\Rightarrow A\in\mathcal{A}(\mathcal{A}\supseteq\mathcal{S}) \Rightarrow X\setminus A\in\mathcal{A}(\mathcal{A}\supseteq\mathcal{S})\Rightarrow X\setminus A\in\mathcal{A}(\mathcal{S})$
\end{proof}
\begin{remark}
    可以发现，在上述证明过程中并未使用到$\mathcal{S}$为半代数的性质，因此上述定义实际上并不依赖于$\mathcal{S}$为半代数.
\end{remark}
类似地，可以定义最小$\sigma-\text{代数}$的概念.
\begin{definition}{最小$\sigma-\!\!$代数}
    ...
    则$\mathcal{B}(\mathcal{A})$是一个$\sigma-\!\!$代数，称为包含$\mathcal{A}$的\textbf{最小$\sigma-\!\!$代数}.
\end{definition}
同样可以证明$\mathcal{B}(\mathcal{A})$为一个$\sigma-\!\!$代数.
\begin{proof}
...
\begin{definition}[有限可加(可列可加)函数]
    ...
    \end{enumerate}
    则称$\tau$是\colorbf{有限可加}的.\par
    如果$n=\infty$，称$\tau$是\colorbf{可列可加}的.
\end{definition}
显然，可列可加函数包含有限可加函数.\par
在代数$\mathcal{A}$和$\sigma-\!\!$代数$\mathcal{B}$上，也可以类似地定义有限可加函数和可列可加函数.例如概率测度空间$(X,\mathcal{B},m)$中的测度$m$是定义在$\sigma-\!\!$代数$\mathcal{B}$上的可列可加函数.\par
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{enumerate}[(1)]
            ...
            所以
            \vspace{-1em}
            {\allowdisplaybreaks[4] % 允许 align 等公式环境跨页中断
                \setjot[-5]\begin{align*}
        ...
        \end{enumerate}
        \item \allbf{函数唯一.}\par
        如果$\tilde{\tau}$也满足条件，则$\tilde{\tau}=\tau_1$  \qedhere
    \end{itemize}
\end{proof}
类似地有代数$\mathcal{A}$到其最小$\sigma-\!\!$代数$\mathcal{B}(\mathcal{A})$的延拓定理.
\begin{theorem}
...
\end{theorem}
\begin{instance}
    设$X=[0,1]$，$\mathcal{S}=\{X\text{的所有子区间}\}$为一个半代数，$\mathcal{B}=\{X\text{中的可测集}\}$是一个$\sigma-\!\!$代数.\par
...
\begin{definition}[单调类][def:monotone-class]
    ...
    \end{enumerate}
    则称$\mathcal{C}$是$X$的一个 \colorbf{单调类}.
\end{definition}
...
对于任意一个集合$X$，它的所有子集构成的$X$的幂集$2^X$一定是包含$X$的一个单调类，因此任何集合都有包含它的单调类.因此，仿照半代数的最小代数、代数的最小$\sigma-\!\!$代数，我们可以定义一个集合生成的最小单调类.
\begin{definition}[最小单调类][def:generated-monotone-class]
    ...
    称为包含$\mathcal{S}$的 \colorbf{最小单调类}.
\end{definition}

下面一个定理介绍了一个代数生成的最小单调类和最小$\sigma-\!\!$代数之间的关系.
\begin{theorem}\label{thm:1.4}
    设$X$是一个集合，$\mathcal{A}$是$X$的一个代数，则$\mathcal{C}(\mathcal{A})=\mathcal{B}(\mathcal{A})$.
\end{theorem}
\begin{proof}
    由于$\mathcal{B}(\mathcal{A})$是一个$\sigma-\!\!$代数，所以$\mathcal{B}(\mathcal{A})$中任意子集的可数交与可数并包含于$\mathcal{B}(\mathcal{A})$，因此$\mathcal{B}(\mathcal{A})$是一个单调类，所以$\mathcal{B}(\mathcal{A})\supseteq\mathcal{C}(\mathcal{A})$.\par
    现证$\mathcal{C}(\mathcal{A})\supseteq\mcal{B}(\mcal{A})$，由于$\mcal{C}(\mcal{A})\supseteq\mcal{A}$，所以只需证明$\mcal{C}(\mcal{A})$是一个$\sigma-\!\!$代数.
    \begin{itemize}
        \item \allbf{取补运算在$\mathcal{C}(\mathcal{A})$中封闭.}\par
        ...
        \item \allbf{交运算在$\mathcal{C}(\mathcal{A})$中封闭.}\par
        ...
        \end{enumerate}
        由(ii)(iii)可得$F\supseteq\mathcal{C}(\mathcal{A})$.结合(i)得$\mathcal{F}= \mathcal{C}(\mathcal{A})$.
        \item \allbf{$\mathcal{C}(\mathcal{A})$是一个$\sigma-\!\!$代数.}\par
        \begin{enumerate}[(1)]
            ...
            $\forall\,A_i$，$A_i^c \in \mathcal{C}(\mathcal{A}) \Rightarrow \bigcap_{i=1}^n{A_i^c} \in \mathcal{C}(\mathcal{A}) \Rightarrow \bigcup_{i=1}^n{A_i}=\left(\bigcap_{i=1}^n{A_i^c}\right)^c \in \mathcal{C}(\mathcal{A})$.\par
            $\left\{\bigcup_{i=1}^n{A_i}\right\}_{n=1}^\infty \in \mathcal{C}(\mathcal{A})$单调递增，所以$\bigcup_{i=1}^\infty{A_i} \in \mathcal{C}(\mathcal{A})$.\qedhere
        \end{enumerate}
    \end{itemize}
\end{proof}
该定理指出要将代数$\mathcal{A}$扩张至其最小$\sigmaalg\mathcal{B}(\mathcal{A})$，只需要将$\mathcal{A}$中单调增集合序列的并集和单调减集合序列的交集包含进来即可.\par
之前在讨论最小代数和最小$\sigma-\!\!$代数时，已经给出了使用半代数中的元素构造其最小代数的方法.但对于代数生成的最小$\sigma-\!\!$代数中的元素，我们却一般无法详细描述，只能使用$\mathcal{A}$中的元素逼近$\mathcal{B}(\mathcal{A})$中的元素。下面我们可以借助$\mathcal{C}(\mathcal{A}) = \mathcal{B}(\mathcal{A})$来证明这种逼近关系.

\begin{theorem}
    设$(X,\mathcal{B},m)$为概率测度空间，$\mathcal{A}$是$X$的一个代数，$\mathcal{B} = \mathcal{B}(\mathcal{A})$，那么$\forall\, B \in \mathcal{B},\forall\,\varepsilon >0,\exists\, A \in \mathcal{A}$使得$m(A\triangle B)<\varepsilon$.
\end{theorem}
\begin{proof}
...
\end{proof}
\begin{remark}
    上述证明过程中除了最后一步实际上并未使用单调集的性质，因此也可以脱离单调集，仅使用$\sigma-\!\!$代数相关概念证明$\mathcal{F} \supseteq \mathcal{B}(\mathcal{A})$.\par
    \begin{proof}
        (1)$\forall B_i \in \mathcal{F} \Rightarrow \bigcup_{i=1}^\infty{B_i} \in \mathcal{F}$.\par
        (2)$B \in \mathcal{F} \Rightarrow \forall \varepsilon>0,\exists A \in \mathcal{A}$，使得$m(A \triangle B)=m(A \setminus B)+m(B \setminus A)=m(B^c \setminus A^c)+m(A^c\setminus B^c)=m(A^c \triangle B^c)<\varepsilon \Rightarrow B^c \in \mathcal{F}$.\par
        结合(1)(2)两点知$\mathcal{F}$为$\sigma-\!\!$代数，所以$\mathcal{F} \supseteq \mathcal{B}(\mathcal{A})$
    \end{proof}
\end{remark}
...
在这个集合上可以定义其半代数.
\begin{definition}[可测矩形][def:measurable-rectangle]
    称
    \begin{equation*}
        \begin{split}
            \mathcal{S}=\bigg\{ \prod_{i=1}^{m-1}{X_i} \times A_m \times A_{m+1} \times \cdots \times A_{n-1} \times A_n \times \prod_{i=n+1}^{+\infty}{X_i} \;\bigg|\; \\
            A_k \in \mathcal{B}_k, m \leqslant k \leqslant n, m,n \in \mathbb{Z} \bigg\}
        \end{split}
    \end{equation*}
    为\colorbf{可测矩形}，它是$X=\prod_{i=-\infty}^{+\infty}{X_i}$的半代数.
\end{definition}

利用该半代数和前面提到的利用半代数生成代数、用代数生成$\sigma-\!\!$代数的方法，我们可以定义$X$上的$\sigma-\!\!$代数.
\begin{definition}[积$\sigma-\!\!$代数][def:product-sigma-algebra]
        定义$\mathcal{B}= \mathcal{B}(\mathcal{A}(\mathcal{S}))$，为$X=\prod_{i=-\infty}^{+\infty}{X_i}$上的$\sigma-\!\!$代数称为\colorbf{积$\sigma-\!\!$代数}.
\end{definition}
类似地，我们可以通过在可测矩形上定义$\tau$函数并将其延拓到$\mathcal{B}(\mathcal{A}(\mathcal{S})))$来得到$(X,\mathcal{B})$上的概率测度.
\begin{definition}[积测度][def:product-measure]
    在$\mathcal{S}$上定义
    \begin{align*}
        \tau:\mathcal{S}&\to[0,1] \\
        \prod_{i=1}^{m-1}{X_i} \times A_m \times \cdots \times A_n \times \prod_{i=n+1}^{+\infty}{X_i} &\mapsto m_m(A_m) \times \cdots \times m_n(A_n)
    \end{align*}
    将$\tau$唯一地延拓到$\mathcal{B}=\mathcal{B}(\mathcal{A}(\mathcal{S}))$，可以得到$\mathcal{B}$上的概率测度，记为$m$，称为\colorbf{积测度}.
\end{definition}
由此，我们就得到了$X$上的$\sigma-\!\!$代数和相应的测度，它们构成了一个概率测度空间.
\begin{definition}[积概率空间][def:product-probability-space] %\label{def:product-probability-space}
    上述的$(X,\mathcal{B},m)$称为$(X_i,\mathcal{B}_i,m_i)$的\colorbf{积概率空间}，记作$\prod_{i=-\infty}^{+\infty}{(X_i,\mathcal{B}_i,m_i})$
\end{definition}
...
\begin{definition}[Borel $\sigma-\!\!$代数]
    设$X=\mathbb{R}$，包含所有开集的最小$\sigma-\!\!$代数称为\colorbf{Borel $\sigma-\!\!$代数$\mathcal{B}(\mathbb{R})$}.
\end{definition}

借助Borel $\sigma-\!\!$代数，可以定义$(X,\mathcal{B},m)$上的可测函数，这里给出其等价定义.
\begin{definition}[可测函数]
    设$(X,\mathcal{B},m)$概率测度空间，对于函数$f:X\to\mathbb{R}$，如果$\forall c \in \mathbb{R}$，$f^{-1}\left((-\infty,c)\right) \in \mathbb{R}$，则称$f$是一个\colorbf{可测函数}.
\end{definition}
...
\begin{definition}[绝对连续]
    设$X$是一个集合，$\mathcal{B}$是$X$的一个$\sigma-\!\!$代数，$\mu,m$是$(X,\mathcal{B})$的概率测度.\par
    如果$\forall B \in \mathcal{B}$，$m(B)=0 \Rightarrow \mu(B)=0$，称$\mu$ \colorbf{绝对连续}于$m$，记为$\mu \ll m$.\par
    如果$\mu \ll m $且$m \ll \mu$，称$\mu$和$m$ \colorbf{等价}.
\end{definition}
...
\begin{proof}
    ...
    $\xLongrightarrow{U_0\text{为开集}} \exists\varepsilon>0,\,B(x_0,\varepsilon) \subseteq U_0$.\par
