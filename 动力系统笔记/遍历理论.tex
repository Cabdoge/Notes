...
\begin{instance}
    ...
    $L^2(X,\mcal{B}_X,m_\mathrm{Leb})$中有一组标准正交基$\big\{e_n=e^{2\pi inx}|n\in\mb{Z}\big\}$，$\| e_n \|=\left(\int_0^1{\left|e^{2\pi inx}\right|^2\d x}\right)^{1/2}=1$.\par
    $\forall\,f\in L^2(X,\mcal{B}_X,m_\mathrm{Leb})$，$\exists\, c_n\in\mb{C},\st f(x)=\sum_{n\in\mb{Z}}{c_n\cdot e_n}\Leftrightarrow \left\|f(x)-\sum_{n=-N}^N{c_n e_n}\right\|_{L^2}\to 0(N\to 0)$.其中$f\in L^2(X,\mcal{B}_X,m_\mathrm{Leb})(f:X\to\mb{C})$，系数$c_n=\int_0^1{f(x)e^{-2\pi inx}}\d x$.\par
    \begin{enumerate}[(1)]
        \item $\alpha\in\mb{Q},\alpha=\frac{n}{m}(m\in\mb{N}^*,n\in\mb{Z},(m,n)=1)$.\par
        则$\exists\, f(x)=e_m=e^{2\pi imx}$，$f\circ R_\alpha(x)=e^{2\pi im(x+\alpha)}=e^{2\pi imx}=f$.\par
        ...
        $f\circ R_\alpha(x)=\sum_{n\in\mb{Z}}{c_n e^{2\pi in(x+\alpha)}}=\sum_{n\in\mb{Z}}{(c_n e^{2\pi in\alpha})\cdot e^{2\pi inx}}$，且$f\circ R_\alpha(x)\in L^2(X,\mcal{B}_X,m_\mathrm{Leb})$.\par
        因为$f\circ R_\alpha=f\aew \Rightarrow f\circ R_\alpha$和$f$代表$L^2(X,\mcal{B}_X,m_\mathrm{Leb})$中的同一元素，因此二者坐标相同，即$c_n=c_n\cdot e^{2\pi in\alpha}$.\par
        由于$\alpha\in\mb{R}\setminus\mb{Q}$，$e^{2\pi in\alpha}\neq 1 \Rightarrow c_n=0(n\neq 0)$.\par
