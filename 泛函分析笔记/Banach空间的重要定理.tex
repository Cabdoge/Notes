...
\begin{definition}[典范等距嵌入]
    ...
    \end{aligned} $ 为\greenbf{典范等距嵌入}.\par
\end{definition}
...
\begin{definition}[自反空间]
    如果 $J_X$ 是满射，则称 $X$ 是一个\bluebf{自反 Banach 空间}.
\end{definition}
...
\begin{definition}[吸收集]
    设 $X$ 是 赋范线性空间，$A \subset X$ 是\greenbf{吸收的} $\iff \forall\, x \in X, \exists\, t > 0, \st\, x \in tA$.
\end{definition}

上述的一个等价描述是 $\bigcup_{t > 0} tA = X$.

\begin{definition}[平衡集]
    设 $X$ 是 $\mb{R}$ 或 $\mb{C}$ 上的赋范线性空间，$B \subseteq X$ 是\greenbf{平衡的} $\iff \forall\, \lambda \in \mb{F}, \st\, |\lambda| \leq 1$, $\lambda B \subset B$.
\end{definition}

下面考虑吸收的，平衡的 $X$ 的凸集 $B$ 应该具有怎样的性质，最直观的感受是它应当类似于单位球.

\begin{definition}[Minkowski 泛函]
    设 $X$ 是赋范线性空间，$A \subset X$ 是吸收集，定义 $\mu_A\colon X \to [0, \infty)$, $x \mapsto \inf \{t > 0 : x \in tA\}$，称为 \bluebf{Minkowski 泛函}.
\end{definition}
...
\begin{definition}[无处稠密，第一纲集，第二纲集]
    \begin{enumerate}[label=(\arabic*)]
        \item 设 $E \subset X$，如果 $\overline{E}$ 没有内点，称 $E$ \greenbf{无处稠密}.
        \item 称 $E$ 是\greenbf{贫乏集}或\greenbf{第一纲集}，当且仅当 $E = \bigcup_{i=1}^\infty N_i$，$N_i$ 无处稠密.
        \item $E \subset X$ 是\greenbf{剩余集}，当且仅当 $X \setminus E$ 是第一纲集.
        \item $E \subset X$ 是\greenbf{第二纲集}，当且仅当 $E$ 不是第一纲集.
    \end{enumerate}
\end{definition}
