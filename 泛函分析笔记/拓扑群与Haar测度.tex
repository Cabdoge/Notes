...
\begin{equation*}
\norm{\pi_t 1_{[0,1]} - 1_{[0,1]}}_{L^p(\mb{R})} = \norm{1_{[t,1]} - 1_{[0,t]}}_{L^p} = (2t)^{\frac{1}{p}} \xrightarrow{t \to 0} 0, \quad \forall\, 1 \leq p < \infty.
\end{equation*}
而 $\norm{1_{[0,t]}}_{L^p} = t^{\frac{1}{p}}$，故 $\norm{\pi_t - \mathrm{Id}} \geq 1$，这表明 $\pi_t$ 在范数拓扑下不收敛到 $\mathrm{Id}$.\par
...
\begin{equation*}
g^{-1}y \in U\overline{N} \subset U \cdot M_{z_i}', \quad \text{for some } 1 \leq i \leq p.
\end{equation*}
\begin{align*}
\Rightarrow |f(g^{-1}y) - f(y)|
&\leq |f(g^{-1}y) - f(z_i)| + |f(z_i) - f(y)| \\
...
称 $X$ 上的测度 $\mu$ 在作用 $G \times X \to X$ 下\colorbf{不变}当且仅当 $\forall\, g \in G$, $g_{\#}\mu = \mu$，称 $\mu$ 是 \colorbf{$G-\!\!$不变的}.
\end{definition}
