...
现在考虑
\begin{align*}
    g_k: \mb{B}_X&\to\mb{K},\\[-6pt]
    x&\mapsto \Phi_{n_k}(Tx) = (T^*f_{n_k})(x).
\end{align*}
...
\begin{enumerate}[(i)]
    \item $\forall\,\lambda\neq0$，特征子空间 $E_\lambda:=\{x\in\mcal{H}: Tx = \lambda x\} = \ker(T - \lambda I)$ 是有限维的.
    \item $\forall\,r>0$，$\{\lambda: |\lambda|\geq r,\; \dim E_\lambda\geq 1\}$ 是有限集.
    \item 所有的特征值都是实数.
\end{enumerate}
...
\begin{lemma}[][lem:self-adjoint-prop]
...
(i) $\mcal{F}(X,Y)$ 是 $\mcal{B}(X,Y)$ 中的开集.\par
(ii) $\Ind:\mcal{F}(X,Y)\to\mb{Z}$连续.
\end{theorem}

\begin{proof}
(i) 设 $T\in\mcal{F}(X,Y)$，取闭子空间 $X_1\subseteq X$，$Y_1\subseteq Y$ 满足上述直和分解.定义
\begin{align*}
...
\end{itemize}
综上，$S\in\mcal{F}(X,Y)$，故 $\mcal{F}(X,Y)$ 是开集.\par
...
\begin{align*}
\dim(Y/\ran S) &= \dim(Y/S(X_1)) - \dim S(Z) \\
&= \dim Y_1 - \dim Z = \dim(Y/\ran T) - \dim Z. \tag{**}
\end{align*}
结合 $(*)$ 和 $(**)$ 得 $\Ind T = \Ind S$.
\end{proof}
...
\begin{align*}
    \Ind: \mcal{F}(X,Y)&\longrightarrow\mb{Z},\\[-6pt]
    T&\longmapsto \dim\ker T - \codim\ran T,
\end{align*}
...
\begin{proof}
先证 $I+K\in\mcal{F}(X)$，分三步验证 Fredholm 算子的三个条件.\par
\fbox{$\dim\ker(I+K) < \infty$} 设 $\{x_n\} \subset \ker(I+K)$，$\norm{x_n}\leq 1$.\par
由 $K$ 是紧算子，存在收敛子列，仍记作 $\{x_n\}$，$Kx_n \to y$. \par
但 $(I+K)x_n = 0 \Rightarrow x_n = -Kx_n \to -y$. 故 $\{x_n\}$ 收敛，由 Riesz 引理知 $\ker(I+K)$ 中的单位球紧，故 $\dim\ker(I+K) < \infty$.\par
\fbox{$\ran(I+K)$ 闭} 将 $X$ 分解为 $X = \ker(I+K) \oplus V$，其中 $V \subseteq X$ 为闭子空间. \par
则 $(I+K)|_V: V \to \ran(I+K)$ 为双射，只需证 $((I+K)|_V)^{-1}$ 在 $0$ 处有界.\par
假设其无界，则 $\exists\,\{x_n\} \subseteq V$，使得 $\norm{x_n} \geq \varepsilon > 0$ 但 $(I+K)x_n \to 0$.\par
令 $y_n = \frac{x_n}{\norm{x_n}}$，则 $\norm{y_n}=1$ 且 $(I+K)y_n \to 0$.\par
由 $K$ 紧，存在子列 $Ky_{n_k} \to z$，故 $y_{n_k} = (I+K)y_{n_k} - Ky_{n_k} \to -z$.\par
于是 $\norm{z}=1$，$z \in V$ 且 $(I+K)z = 0$，与 $\ker(I+K) \cap V = \{0\}$ 矛盾.\par
\fbox{$\codim\ran(I+K) < \infty$} 若不然，存在一列闭子空间
\begin{equation*}
\ran(I+K) = H_0 \subsetneq H_1 \subsetneq \cdots \subsetneq X,\quad \st \dim(H_{n}/H_{n-1}) = 1.
\end{equation*}
由 Riesz 引理，$\forall\, n\in\mb{N}$，可取 $x_n \in H_n$，$\norm{x_n}=1$，使得 $\forall\, y \in H_{n-1},\norm{x_n - y} \geq \frac{1}{2}$.\par
对 $1 \leq \ell \leq n-1$，
\begin{align*}
\norm{Kx_n - Kx_\ell}
&= \norm{(x_n+Kx_n) - (x_\ell+Kx_\ell) - x_n + x_\ell}\\
&= \norm{(I+K)(x_n) - (I+K)(x_\ell) - (x_n-x_\ell)}\\
&\geq 0+0+\frac{1}{2}=\frac{1}{2},
\end{align*}
其中最后一步用到 $(I+K)x_n, (I+K)x_\ell \in H_0 \subseteq H_{n-1}$. 这与 $K$ 紧矛盾.\par
综上，$I+K \in \mcal{F}(X)$.\par
...
\begin{proof}
\fbox{$\Rightarrow$} 设 $T \in \mcal{F}(X,Y)$，有 $X = \ker T \oplus X_1$，$Y = \ran T \oplus Y_1$，其中 $\dim Y_1 < \infty$.取
\begin{equation*}
...
\fbox{$\Leftarrow$} 设 $\exists\, T_1 \in \mcal{B}(Y,X)$ 使得
\begin{equation*}
...
\end{align*}
由于 $I_X-K_1$，$I_Y-K_2$ 均为 Fredholm 算子，故 $\dim\ker(I_X-K_1) < \infty$，$\codim\ran(I_Y-K_2) < \infty$.\par
从而 $\dim\ker T < \infty$，$\codim\ran T < \infty$，且 $\ran T$ 闭，故 $T$ 是 Fredholm 算子.
\end{proof}
...
\end{cases}
\end{equation*}
相减得 $a_2\ab(1-\frac{\lambda_2}{\lambda_1})x_2 + \cdots + a_n\ab(1-\frac{\lambda_n}{\lambda_1})x_n = 0$. \par
...
\begin{proof}
\textbf{(i)} 取 $\lambda \in \sigma(T)\setminus\{0\}$，则 $T-\lambda I$ 为 Fredholm 算子，指标为 $0$，但不可逆. \par
若 $\ker(T-\lambda I) = \{0\}$，则其为单射，结合指标为零知 $\ran(T-\lambda I) = X$，由开映射定理 $T-\lambda I$ 可逆，矛盾. 故 $\ker(T-\lambda I) \neq \{0\}$，即 $\lambda$ 为特征值.\par
\textbf{(ii)} 若 $0 \notin \sigma(T)$，则 $T$ 是可逆紧算子.\par
 故 $I_X = TT^{-1}$ 也为紧算子，由 Riesz 引理知 $\dim X < \infty$.\par
\textbf{(iii)} 只需证明 $\forall\, R>0$，集合 $\{\lambda \in \sigma(T): |\lambda| \geq R\}$ 有限.\par
若不然，存在互异特征值 $\lambda_1, \lambda_2, \dots$ 使得 $\forall\, j,|\lambda_j| \geq R$ . \par
设 $x_1, x_2, \dots$ 为对应的特征向量，令 $H_n = \spann\{x_1,x_2,\dots,x_n\}$，则
\begin{equation*}
H_1 \subsetneq H_2 \subsetneq H_3 \subsetneq \cdots \subseteq X,\quad \dim(H_n/H_{n-1}) = 1\ (\forall\, n \in \mb{N}).
\end{equation*}
由 Riesz 引理，$\exists\, w_1, w_2, \dots$ 使得 $w_n \in H_n$ 且 $\forall\, y \in H_{n-1},\norm{w_n - y} \geq \frac{1}{2}$，$\norm{w_n}=1,\forall\, n \in \mb{N}$.\par
下证 $\{Tw_n\}$ 没有收敛子列.\par
取定 $n > \ell \in \mb{N}$，$w_n = a_n x_n + z_{n-1}$，其中 $z_{n-1} \in H_{n-1}$. 则
\begin{align*}
\norm{Tw_n - Tw_\ell}
...
N_0 = \{x \in X: f(x)=0,\ \forall f \in N\},
\end{equation*}
分别为零化子 与预零化子. 则，
\begin{equation*}
\begin{cases}
\dim \ker(T-\lambda I) = \dim \ker(T^*-\lambda I) < +\infty,\\[4pt]
\ran(T-\lambda I) = [\ker(T^*-\lambda I)]_0,\\[4pt]
...
设 $\{f_j\}$ 是 $Y^*$ 中稠密序列，由对角化技巧，存在子列 $\{x_{n_k}\} \subset \{x_n\}$ 使得 $\forall\, j \in \mb{N}\{f_j(x_{n_k})\}_{k \in \mb{N}}$ 收敛.\par
由 $J: X \xrightarrow{\cong} X^{**}$，则 $f(x_{n_k}) = J(x_{n_k})(f) \xrightarrow{k \to \infty} \xi(f)$，对某个 $\xi \in X^{**}$. 这等价于
\begin{equation*}
...
\begin{enumerate}[(i)]
    \item 称 $I$ \colorbf{序列弱下半连续}，当且仅当$x_n \rightharpoonup x$ 时恒有 $I(x) \leq \liminf_{n \to \infty} I(x_n)$.
    \item 称 $I$ \colorbf{强制}，当且仅当$\|x_n\| \to \infty$ 时恒有 $I[x_n] \to \infty$.
\end{enumerate}
\end{definition}
...
\begin{proof}
取极小化序列 $\{u_n\} \subset X$，即 $I[u_n] \searrow \inf_{u \in X} I[u]$.\par
由 $I$ 的强制性，$\{u_n\}$ 有界.\par
...
\begin{proof}
任取 $x_n \to x$ in $X$. \par
由于 $\{x_n\}$ 在范数意义下收敛且$I$ 凸，存在子列 $\{x_{n_k}\},\st I[x_{n_k}] \to \liminf_{n \to \infty} I[x_n] < \infty$.\par
...
&= \liminf_{n \to \infty} I[x_n].\qedhere
\end{align*}
\end{proof}
