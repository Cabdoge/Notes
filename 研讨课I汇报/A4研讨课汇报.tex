...
\begin{frame}{多面体的半空间表示}
    \begin{proposition}{}
    有界多面体$P$都有使用半空间的交集的对偶表示：
    $P=\{x:s_i^Tx\leqslant r_i,\forall\, i\}.$
  \end{proposition}
...
\begin{frame}{使用二元变量表示区域}
  为了便于描述上述区域，可以引入二元变量$(\alpha_1,\cdots,\alpha_N)\in\{0,1\}^N$，使用混合整数技术进行表示:\par
  \begin{subequations}
    \label{7}
    \begin{gather}
...
\begin{frame}{任务分配}
  ...
  \begin{subequations}
    \begin{align}
      ...
      &\sum_{j=1}^M{x_{ij}}\geqslant 1,\forall\, i, \\
...
\begin{frame}{路径}
  \begin{definition}[路径]
    ...
    \end{equation}
  \end{definition}
  一般有两种方式描述函数$\gamma$，其一为显式方法，包括为整个区间$[0,1]$提供$\gamma$的值.其二是更常用的方法，函数必须从被称为“航点”的值中取值.\par
...
\begin{frame}{路径规划}
  ...
  \begin{example}[Richards,How(2002)、Schouwenaars(2006)]
    ...
    \begin{subequations}
      ...
      \end{gather}
    \end{subequations}
    其中$T$是时间步的数量，$b_{ik}$是二元变量，表示在时间步$i$时是否访问了航点$k$.
  \end{example}
\end{frame}
...
\begin{frame}{路径规划的其他方法}
  \begin{itemize}
    ...
    \begin{enumerate}
      ...
      \item 使用Delaunary三角剖分将空间划分为三角形，并计算被规划路径穿过的三角形，将其合并形成多图面体，寻找隧道中的途径凸区域数量最少的路径
    \end{enumerate}
  \end{itemize}
\end{frame}
...
\begin{frame}{轨迹}
  \begin{definition}[轨迹]
    导航空间中两个点$x_0,x_f$之间的轨迹由连续函数$\gamma:[t_0,t_f]\to \mathcal{X}$给出，其中
    \begin{equation}
...
\begin{frame}{各种任务中的轨迹规划}
  \begin{itemize}
    \item Earl和D'Andrea(2005a)：提出一种迭代MILP算法，保证车辆在轨迹上保证避开\hl{圆形障碍物}.
    \item Cetin、Bikdash和Hadaegh(2007)：解决了航天器编队重构生成无碰撞轨迹.使用三次样条将轨迹\hl{时间离散化}.
    \item Richards和How(2003)：使用MILP进行开环车辆轨迹设计，从而可以\hl{包含非凸约束}，例如避免羽流撞击
    \item Molinari(2017)：为自主超车问题引入了一种高效的MIP公式，在IPG CarMaker中\hl{得到验证}.
    \item Ballesteros-Tolosana(2017)：针对确保乘客舒适度的变道和超车操作的轨迹生成问题，使用\hl{多重shooting方法}求解枚举所有超车配置及其产生的紧凑型MIP公式，\hl{计算负担有所改进}.
  \end{itemize}
\end{frame}
...
\begin{frame}{自动驾驶任务中的轨迹规划}
  \begin{itemize}
    \item Fayazi(2017)：目标是找到车辆穿过交叉路口最优序列及其访问时间.通过MIP建模，将总优化成本由一个min-max问题和含有绝对值的\hl{非线性成本}组成.
    \item 黄和彭(2017)：优化交叉口的车辆速度轨迹，最小化燃油消耗和行驶时间.用\hl{整数变量模拟了激活的绿灯时段}.
...
\begin{frame}{静态障碍物}
  \begin{itemize}
    \item Culligan(2006)：使用MILP解决了无人机的递推预测控制优化问题，包括障碍物和多车辆规避的硬约束和车辆动力学近似，并\hl{全面考虑3D情况}.
    \item Maia、Galvao(2009)，Richards、Turnball(2013)：考虑额外约束确保两个连续位置之间\hl{不会穿过障碍物}.
    \item Stoican，Prodan和Grotli(2018)：在\hl{超平面排列}设置中对多障碍物情况进行处理，使用了精确以及近似的表示
  \end{itemize}
\end{frame}
...
\begin{frame}{连通性维护和编队控制}
  ...
  \end{block}
  \begin{enumerate}
    \item 领导者-跟随者方法：指定一个智能体作为领导者以特定方式移动，其余智能体跟踪领导者在其周围保持编队
...
\begin{frame}{Maia,Galvao(2009)}
  ...
  \end{equation}
  对于所有可能的$\sigma^\circ\in\{0,1\}^N$，至少有一个约束(\ref{31})会被简化为(\ref{30})，即当$\sigma^\circ=\sigma^+$时.
\end{frame}
...
\begin{frame}{由外部约束引起的连通性}
  ...
  \vspace{1 em}
  Ritter等人(2014)通过由一组配备传感器的自主车辆组成的移动传感器平台解决了该问题，提出了一种自适应观测策略.\par
  该策略基于过程模拟和多智能体写作系统，使用MLD系统建模，并用PDE估计代理的目标位置.
\end{frame}
...
\begin{frame}{集中式控制架构的应用}
  \begin{itemize}
    ...
    \item 对于全局场景，所有智能体掌握彼此动态，因此可以\hl{使用非凸约束实现碰撞避障}.
    \item Wang(2015)：研究了多智能体系统中的碰撞避免，将\hl{碰撞避免问题转化为了资源分配问题}，使用MIP解决相关优化问题.
  \end{itemize}
\end{frame}

\begin{frame}{分布式和去中心化控制架构的应用}
  \begin{itemize}
    \item Schouwenaars(2006)：采用分布式MPC策略导航车队穿越部分位置和杂乱的环境
    \item Testa,Rucco,Notarstefano(2017)：提出了用于求解MILP的算法，约束分布在各个智能体之间
    \item Mariéthoz, and Morari (2016)：基于\hl{拉格朗日对偶}，提出了对大规模MILP特别有效的分解办法
    \item Van Parys and Pipeleers (2016)：使用\hl{交替方向乘子法}构建问题
  \end{itemize}
  \begin{itemize}
    \item Bali and Richards(2018)：提出了合并交叉路口问题，根据定义的标准对多个到达交叉路口的智能体进行有限级排序
  \end{itemize}
\end{frame}
...
\begin{frame}{MIP求解方法和启发式技术}
    ...
    \end{table}
  \begin{itemize}
    \item ADMN:常用于最优功率流问题.
    \item FP：用于寻找给定MIP可行解的启发式算法，旨在最小化LP松弛解与原始MIP解之间的差异.
  \end{itemize}
\end{frame}
...
\begin{frame}{基于图的算法}
  ...
  \begin{enumerate}
    ...
    \begin{itemize}
      ...
      \item RRT：更适用于环境未知的场景，图的构建和路径搜索\hl{同时进行}，得到的图是树状的
    \end{itemize}
  \end{enumerate}
\end{frame}


\begin{frame}{势场公式和凸松弛}
  势场公式通过给成本中添加罚函数来添加约束.它依赖于构造一个标量函数，使得当智能体停留在禁止区域时，函数取高值，以此使函数沿势的负梯度方向移动到最终点.
    \begin{block}{凸松弛}
...
\begin{frame}{连续凸化和时变约束}
  连续凸化的基本思想是通过一系列凸子问题解决非凸最优控制问题.\par
  针对于来自非线性动力学或非凸约束的非凸性，都是用线性化来处理，通常使用一阶泰勒展开近似.\par
  \vspace{1 em}
  时变约束是指将非凸域分解为一系列凸区域，并引入切换时刻.在每个时刻，智能体都应停留在某个凸区域内.\par
