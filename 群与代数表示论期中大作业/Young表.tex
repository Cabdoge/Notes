...
\begin{definition}[分拆]\index{f@分拆 (partition)}\index{f@分拆数 $\rho(n)$}
...
\begin{theorem}[$S_n$的共轭类数]{\label{thm:conjugacy_classes_of_Sn}}\index{d@对称群 $S_n$}\index{g@共轭类与轮换类型}
...
\begin{definition}[Young图]\index{y@Young 图}
...
\begin{definition}[Young表]\index{y@Young 表}
...
\begin{definition}[标准Young表]\index{b@标准Young表}
...
\begin{theorem}[钩长公式]\index{g@钩长公式 (hook length formula)}\index{g@钩长 $h_{i,j}$}
...
\begin{definition}[Young子群]\index{y@Young 子群}
...
\begin{definition}[行群与列群]\index{h@行群 $R_t$}\index{l@列群 $C_t$}
...
\begin{definition}[Young对称化子]\index{y@Young 对称化子 $c_t$}\index{h@行对称化子 $a_t$}\index{l@列反对称化子 $b_t$}
...
\begin{lemma}[von Neumann引理]{\label{lem:von_neumann}}\index{v@von Neumann 引理}
